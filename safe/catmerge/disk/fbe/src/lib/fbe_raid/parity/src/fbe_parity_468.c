...
/*!*************************************************************************
 * @file fbe_parity_468.c
 ***************************************************************************
 *
 * @brief
 *  This file contains functions for the parity 468 write algorithm.
 * 
 *  This is a small write algorithm where parity will pre-read parity and
 *  the data being written.  Parity will also write parity and the host data.
 * 
...
fbe_raid_state_status_t fbe_parity_small_468_state0(fbe_raid_siots_t * siots_p)
{
...
#if 0
    {
        ...
        fbe_transport_get_packet_attr(iots_p->packet_p, &packet_attr);
        if(packet_attr & FBE_PACKET_FLAG_COMPLETION_BY_PORT)
        {
            fbe_raid_siots_transition(siots_p, fbe_parity_468_state3);
            fbe_parity_small_468_setup_embed_resources(siots_p);
            return FBE_RAID_STATE_STATUS_EXECUTING;
        }
    }
#endif
     ...
     */
    status = fbe_parity_small_468_calculate_memory_size(siots_p, &sg_type);
    if (status != FBE_STATUS_OK)
    {
        fbe_raid_siots_set_unexpected_error(siots_p,
                                            FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
                                            "parity: status of 0x%x from calculate 468 memory size.\n",
                                            status);
        return FBE_RAID_STATE_STATUS_EXECUTING;
    }
     ...
     */
    fbe_raid_siots_transition(siots_p, fbe_parity_small_468_state1);
    ...
    if (RAID_MEMORY_COND(status == FBE_STATUS_OK))
    {
         ...
         */
        status = fbe_parity_small_468_setup_resources(siots_p, sg_type);
        if (RAID_COND(status != FBE_STATUS_OK))
        {
            fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
                                                "parity: 468 failed to setup resources. status: 0x%x\n",
                                                status);
            return FBE_RAID_STATE_STATUS_EXECUTING;
        }
         ...
         */
        fbe_raid_common_set_state(&siots_p->common, (fbe_raid_common_state_t)fbe_parity_468_state3);
    }
    ...
    else
    {
         ...
         */
        fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
                                            "parity: 468 memory request failed with status: 0x%x\n",
                                            status);
        return FBE_RAID_STATE_STATUS_EXECUTING;
    }
...
/*******************************
 * end fbe_parity_small_468_state0()
 *******************************/
/*!**************************************************************
 * fbe_parity_small_468_state1()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_small_468_state1(fbe_raid_siots_t * siots_p)
{
    ...
    if (RAID_MEMORY_COND(!fbe_raid_siots_is_deferred_allocation_successful(siots_p)))
    {
        ...
        fbe_raid_siots_trace(siots_p, FBE_RAID_SIOTS_TRACE_PARAMS_ERROR,
                             "parity: 468 memory allocation error common flags: 0x%x \n",
...
/*******************************
 * end fbe_parity_small_468_state1()
 *******************************/
/*!**************************************************************
 * fbe_parity_small_468_state2()
 ****************************************************************
 * @brief
 *  When the small 468 takes any error, we will start over
 *  by reusing the siots (freeing resources) and then
 *  kicking it back into the normal 468 state machine.
 *
 * @param siots_p - current sub request
 *
 * @return  This function returns FBE_RAID_STATE_STATUS_EXECUTING
 *
 * @author
 *  2/14/2012 - Created. Rob Foley
 *
 ****************************************************************/

fbe_raid_state_status_t fbe_parity_small_468_state2(fbe_raid_siots_t * siots_p)
{
    ...
    /* We need to fill this in since it was not filled in originally to save time.
     */
    fbe_parity_get_lun_geometry(raid_geometry_p, siots_p->start_lba, &siots_p->geo);

    return FBE_RAID_STATE_STATUS_EXECUTING;
}
/*******************************
 * end fbe_parity_small_468_state2()
 *******************************/
/****************************************************************
 * fbe_parity_468_state0()
 ****************************************************************
 * @brief
 *  Allocate all resources required.
 *
...
fbe_raid_state_status_t fbe_parity_468_state0(fbe_raid_siots_t * siots_p)
{
    ...
    /* Validate the algorithm.
     */
    if (RAID_DBG_COND((siots_p->algorithm != R5_468) &&
    ...
    /* Calculate and allocate all the memory needed for this siots.
     */
    status = fbe_parity_468_calculate_memory_size(siots_p, &read_info[0], &write_info[0], &num_pages);
    if (status != FBE_STATUS_OK)
    {
        fbe_raid_siots_set_unexpected_error(siots_p,
                                            FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
     ...
     *  o Other - Memory allocation error
     */
    fbe_raid_siots_transition(siots_p, fbe_parity_468_state1);
    status = fbe_raid_siots_allocate_memory(siots_p);
    ...
    if (RAID_MEMORY_COND(status == FBE_STATUS_OK))
    {
         ...
         * transition to next state.
         */
        status = fbe_parity_468_setup_resources(siots_p, &read_info[0], &write_info[0]);
        if (RAID_COND(status != FBE_STATUS_OK))
        {
            fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
                                                "parity: 468 failed to setup resources. status: 0x%x\n",
...
/*******************************
 * end fbe_parity_468_state0()
 *******************************/

/****************************************************************
 * fbe_parity_468_state1()
 ****************************************************************
 * @brief
 *  Resources weren't immediately available.  We were called back
 *  by the memory service.
...
/*******************************
 * end fbe_parity_468_state1()
 *******************************/

/****************************************************************
 *  fbe_parity_468_state3()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state3(fbe_raid_siots_t * siots_p)
{
    ...
    siots_p->wait_count = siots_p->data_disks + parity_drives;
    ...
    {
        fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
                                            "parity: 468 wait count unexpected %lld\n",
                                            siots_p->wait_count);
        return FBE_RAID_STATE_STATUS_EXECUTING;
    }

    /* If the raid group is degraded, we need to check corrupt data op */
    if ((opcode == FBE_PAYLOAD_BLOCK_OPERATION_OPCODE_CORRUPT_DATA) &&
        (fbe_raid_fruts_check_degraded_corrupt_data(fruts_p, siots_p)))
    {
        /* We are writing corrupt crc to degraded position, exit now */
    ...
    if (!fbe_raid_iots_is_flag_set(iots_p, FBE_RAID_IOTS_FLAG_NON_DEGRADED))
    {
        ...
        if (RAID_COND(status != FBE_STATUS_OK))
        {
             ...
             */
            fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
                                                "parity: 468 error checking preread degraded\n");
            return FBE_RAID_STATE_STATUS_EXECUTING;
        }
    }
    ...
    else
    {
        ...
        {
            fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS, 
                                                "parity 468 failed in next siots generation: siots_p = 0x%p, status = %dn",
                                                siots_p, status);
            return(FBE_RAID_STATE_STATUS_EXECUTING);
        }
    }
    ...
    if (b_preread_degraded == FBE_FALSE)
    {
         ...
         *************************************************************/
        fbe_raid_siots_transition(siots_p, fbe_parity_468_state6);
    ...
    else
    {
        ...
        {
            /* Small 468s do not handle pre-read errors.  Restart this as a normal 468.
             */
            fbe_raid_siots_transition(siots_p, fbe_parity_small_468_state2);
            return FBE_RAID_STATE_STATUS_EXECUTING;
        }        
         ...
         */
        fbe_raid_siots_transition(siots_p, fbe_parity_468_state15);
...
/*******************************
 * end fbe_parity_468_state3()
 *******************************/
/****************************************************************
 * fbe_parity_468_state6()
 ****************************************************************
 * @brief
...
{
    /* If we get an error the error pending flag will be set by fruts completion fn.
     */
    if (!fbe_raid_siots_is_flag_set(siots_p, FBE_RAID_SIOTS_FLAG_ERROR_PENDING))
    {
        /* All pre-reads successful.
         */
        fbe_raid_siots_transition(siots_p, fbe_parity_468_state7);
        return FBE_RAID_STATE_STATUS_EXECUTING;
    }
    else
    {
         ...
         */
        if (status == FBE_RAID_FRU_ERROR_STATUS_WAITING)
        {
         ...
         * errors. 
         */
        else if ((eboard.hard_media_err_count > 0) && (status == FBE_RAID_FRU_ERROR_STATUS_RETRY))
        {
            status = FBE_RAID_FRU_ERROR_STATUS_ERROR;
        }

        if ((status != FBE_RAID_FRU_ERROR_STATUS_RETRY) &&
            (siots_p->algorithm == R5_SMALL_468))
        {
            /* Small 468s do not handle pre-read errors other than retry.  Restart this as a normal 468.
             */
            fbe_raid_siots_transition(siots_p, fbe_parity_small_468_state2);
            return FBE_RAID_STATE_STATUS_EXECUTING;
        }
        ...
        {
            if ((eboard.dead_err_count <= fbe_raid_siots_parity_num_positions(siots_p)) &&
                (eboard.dead_err_count > 0))
            {
                ...
                /* Corrupt CRC don't write to parity. If we hit a dead position,
                 * return media error for consistency.
                 */
                if ((opcode == FBE_PAYLOAD_BLOCK_OPERATION_OPCODE_CORRUPT_DATA) &&
                    (fbe_raid_fruts_check_degraded_corrupt_data(write_fruts_ptr, siots_p)))
                {
                    /* We are writing corrupt crc to degraded position, exit now */
                    fbe_raid_siots_transition(siots_p, fbe_raid_siots_media_error);
                }
            ...
            {
                /* A pre-read cannot fail due to a dropped.
                 * We should not submit optional pre-reads.
                 */
                fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
            ...
            else if (eboard.hard_media_err_count > 0)
            {
                 ...
                 * a hard error; shift to verify recovery.
                 */
                fbe_raid_siots_transition(siots_p, fbe_parity_468_state16);
            }
            else
            {
...
/*******************************
 * end fbe_parity_468_state6()
 *******************************/

/****************************************************************
 * fbe_parity_468_state7()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state7(fbe_raid_siots_t * siots_p)
{
    ...
    if (!fbe_raid_siots_is_aborted(siots_p))
    {
        fbe_raid_siots_transition(siots_p, fbe_parity_468_state8);
        ...
        if (fbe_raid_siots_is_buffered_op(siots_p))
        {
            ...
            if (status != FBE_STATUS_OK)
            {
                fbe_raid_siots_set_unexpected_error(siots_p, __LINE__, "parity_468_state7" ,
                                                    "fbe_raid_xor_get_buffered_data returned stat: %d\n", status);
                return FBE_RAID_STATE_STATUS_EXECUTING;
            }
        }

        /* Kick off the 468 xor.
         */
        status = fbe_parity_468_xor(siots_p);
        if (status != FBE_STATUS_OK)
        {
            fbe_raid_siots_set_unexpected_error(siots_p, __LINE__, "parity_468_state7",
                                                "parity_468_state7 fail to perform 468 xor operation:"
...
/*******************************
 * end fbe_parity_468_state7()
 *******************************/

/****************************************************************
 * fbe_parity_468_state8()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state8(fbe_raid_siots_t * siots_p)
{
    ...
    if (xor_status == FBE_XOR_STATUS_NO_ERROR)
    {
        ...
        if (fbe_raid_siots_dead_pos_get(siots_p, FBE_RAID_FIRST_DEAD_POS) == FBE_RAID_INVALID_DEAD_POS)
        {
             ...
             */
            fbe_raid_siots_transition(siots_p, fbe_parity_468_state13);
        }
        else
        {
            /* Use the opcode to check for the Corrupt Data operation.  If we are degraded,
            ...
            if (opcode == FBE_PAYLOAD_BLOCK_OPERATION_OPCODE_CORRUPT_DATA )
            {
                ...
                fbe_raid_siots_get_write_fruts(siots_p, &fruts_p);
                if ((fbe_raid_fruts_check_degraded_corrupt_data(fruts_p, siots_p)))
                {
                    /* We are writing corrupt data to degraded position, since we don't 
                     * touch parity, exit now with media error. 
                     */
                    fbe_raid_siots_transition(siots_p, fbe_raid_siots_media_error);
                }
                else
                {
                    /* The write position is good, continue with normal write. */
                    fbe_raid_siots_transition(siots_p, fbe_parity_468_state13);
                }
            }
            else if (!fbe_parity_is_write_logging_required(siots_p))
            {
            ...
            else
            {
                ...
                if (fbe_raid_siots_is_flag_set(siots_p, FBE_RAID_SIOTS_FLAG_WRITE_LOG_HEADER_REQD))
                {
                    fbe_raid_siots_transition(siots_p, fbe_parity_468_state9);
                }
                else
                {
                    /* Abort the request so that its retried as a degraded-IO and
    ...
    else if (siots_p->algorithm == R5_SMALL_468)
    {
        /* Small 468s do not handle pre-read errors.  Restart this as a normal 468.
         */
        fbe_raid_siots_transition(siots_p, fbe_parity_small_468_state2);
             ...
             (FBE_XOR_STATUS_BAD_METADATA | FBE_XOR_STATUS_BAD_SHEDSTAMP))
    {
        ...
        {
            /*********************************************************
             * In the past we would fbe_panic with a RG_468_FOUND_SHED_STAMPS
             * panic.  However, now we tolerate this error and
...
#if 0
        /* Increment our counter of FOUND_SHED_STAMP errors.
         */
        RG_INC_ERROR_STATS(fbe_raid_siots_get_raid_geometry(siots_p)->group, RG_STATS_FOUND_SHED_STAMPS);
#endif        
...
/*******************************
 * end of fbe_parity_468_state8()
 *******************************/
/****************************************************************
 * fbe_parity_468_state9()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state9(fbe_raid_siots_t * siots_p)
{
    ...
    fbe_raid_siots_transition(siots_p, fbe_parity_468_state10);
    ...
    {
        fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
                                            "Parity: 468 write log init cpu affinity packet unexpected status: 0x%x\n",
                                            fbe_status);
        return FBE_RAID_STATE_STATUS_EXECUTING; 
    }
...
/*******************************
 * end fbe_parity_468_state9()
 *******************************/
/****************************************************************
 * fbe_parity_468_state10()
 ****************************************************************
 * @brief 
 *  Need to write to the write log before completing the write.
...
fbe_raid_state_status_t fbe_parity_468_state10(fbe_raid_siots_t * siots_p)
{
    ...
    {
        fbe_raid_siots_set_unexpected_error(siots_p, FBE_RAID_SIOTS_TRACE_UNEXPECTED_ERROR_PARAMS,
                                            "parity: 468 wait_count unexpected %lld\n",
                                            siots_p->wait_count);
     ...
     */
    fbe_raid_siots_transition(siots_p, fbe_parity_468_state11);
...
/*******************************
 * end fbe_parity_468_state10()
 *******************************/
/****************************************************************
 * fbe_parity_468_state11()
 ****************************************************************
 * @brief
 *  Need to write to the write log before completing the write.
...
fbe_raid_state_status_t fbe_parity_468_state11(fbe_raid_siots_t * siots_p)
{
    ...
    if (fru_status == FBE_RAID_FRU_ERROR_STATUS_SUCCESS)
    {
         ...
         */
        fbe_raid_siots_transition(siots_p, fbe_parity_468_state12);
    }
    ...
    else if (fru_status == FBE_RAID_FRU_ERROR_STATUS_DEAD)
    {
        ...
        fbe_raid_siots_transition(siots_p, fbe_parity_468_state26);
    }
...
/*******************************
 * end fbe_parity_468_state11()
 *******************************/

/****************************************************************
 * fbe_parity_468_state12()
 ****************************************************************
 * @brief
...
/*******************************
 * end fbe_parity_468_state12()
 *******************************/
/****************************************************************
 * fbe_parity_468_state13()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state13(fbe_raid_siots_t * siots_p)
{
    ...
    else
    {
        ...
        /* If this is a corrupt data, set all parity positions to be a no-op.
         * Use the opcode to check for the Corrupt Data operation.  If we are degraded,
         * the algorithm may have changed.
         */
        fbe_raid_siots_get_opcode(siots_p, &opcode);
        if (opcode == FBE_PAYLOAD_BLOCK_OPERATION_OPCODE_CORRUPT_DATA )
        {
            fbe_raid_fruts_set_parity_nop(siots_p, fruts_ptr);
        }
    ...
    fbe_raid_siots_transition(siots_p, fbe_parity_468_state14);
...
/*******************************
 * end og fbe_parity_468_state13
 *******************************/

/****************************************************************
 * fbe_parity_468_state14()
 ****************************************************************
 * @brief
...
/*******************************
 * end og fbe_parity_468_state14()
 *******************************/

/****************************************************************
 * fbe_parity_468_state15()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state15(fbe_raid_siots_t * siots_p)
{
     ...
     * set our algorithm to R5_DEG_468, to help note that we have
     ...
     */
    fbe_raid_siots_transition(siots_p, fbe_parity_468_state22);
...
/*******************************
 * end of fbe_parity_468_state15()
 *******************************/

/****************************************************************
 * fbe_parity_468_state16()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state16(fbe_raid_siots_t * siots_p)
{
     ...
     * continue with write state machine.
     */
    fbe_raid_siots_transition(siots_p, fbe_parity_468_state19);
...
/*******************************
 * end of fbe_parity_468_state16()
 *******************************/

/****************************************************************
 * fbe_parity_468_state18()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state18(fbe_raid_siots_t * siots_p)
{
    ...
    if ((fbe_raid_geometry_is_raid6(fbe_raid_siots_get_raid_geometry(siots_p))) && 
        (RG_GET_IORB(siots_p)->opcode == IORB_OP_CORRUPT_CRC))
    {
        /* Report invalidations.
         */
        r6_corrupt_crc_report(siots_p);
    }
#endif

    fbe_raid_siots_transition(siots_p, fbe_raid_siots_success);

    /* Small 468s don't handle errors and don't have eboards.
     */
    if (siots_p->algorithm != R5_SMALL_468)
    {
...
/*******************************
 * end of fbe_parity_468_state18()
 *******************************/

/****************************************************************
 *  fbe_parity_468_state19()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state19(fbe_raid_siots_t * siots_p)
{
    ...
    else if (siots_p->error == FBE_PAYLOAD_BLOCK_OPERATION_STATUS_SUCCESS)
    {
         ...
         * continue with write.
         */
        fbe_raid_siots_transition(siots_p, fbe_parity_468_state18);
    }
...
/*******************************
 * end of fbe_parity_468_state19()
 *******************************/

/***************************************************************
 *  fbe_parity_468_state22()
 ****************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state22(fbe_raid_siots_t * siots_p)
{
    ...
    {
        if (siots_p->qualifier == FBE_PAYLOAD_BLOCK_OPERATION_QUALIFIER_RETRY_POSSIBLE)
        {
            /* There is a retryable error, go dead error
            */
...
/**************************
 * end of fbe_parity_468_state22
 **************************/ 

/******************************************************************************
 * fbe_parity_468_state26()
 ******************************************************************************
 * @brief
...
fbe_raid_state_status_t fbe_parity_468_state26(fbe_raid_siots_t * siots_p)
{
    ...
    if (write_log_slot_id != FBE_RAID_INVALID_JOURNAL_SLOT)
    {
        ...
        /* set the next state */
        fbe_raid_siots_transition(siots_p, fbe_parity_468_state27);
...
/*******************************************************************************
 * end fbe_parity_468_state26()
 ******************************************************************************/

/******************************************************************************
 * fbe_parity_468_state27()
 ******************************************************************************
 * @brief
...
/*******************************************************************************
 * end fbe_parity_468_state27()
 ******************************************************************************/

/*************************************************************
 * end of file fbe_parity_468.c
 *************************************************************/
