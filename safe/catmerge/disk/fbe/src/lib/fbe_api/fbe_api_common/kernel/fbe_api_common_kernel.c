...
/*!*************************************************************************
 * @file fbe_api_common_kernel.c
 ***************************************************************************
 *
 * @brief
 *   Kernel implementation for common API interface.
 *      
...
static fbe_status_t fbe_api_common_destroy_contiguous_packet_queue_kernel(void);
static fbe_status_t fbe_api_common_init_contiguous_packet_queue_kernel(void);
...
fbe_status_t FBE_API_CALL fbe_api_common_init_kernel ()
{
	...
	/*befoer allocating contiguous memory, we need to know what kind of driver we have*/
    status  = fbe_api_common_init_contiguous_packet_queue_kernel();
	if (status != FBE_STATUS_OK) {
		return status;
	}
...
fbe_status_t FBE_API_CALL fbe_api_common_destroy_kernel (void)
{
    ...
    fbe_api_common_kernel_init_reference_count --;
    fbe_api_common_destroy_contiguous_packet_queue_kernel();
...
connect_to_control_and_io_entries(fbe_package_id_t package_id)
{
    ...
    PEMCPAL_FILE_OBJECT			file_object;
    ...
    if (!EMCPAL_IS_SUCCESS(status)) {
        fbe_base_library_trace(FBE_LIBRARY_ID_FLARE_SHIM,
                               ...
                               "%s: EmcpalDeviceOpen failed for %s, might not be present\n",
			       __FUNCTION__,
			       packages_handles[package_id].driver_link_name); 
...
static void
disconnect_control_and_io_entries(fbe_package_id_t package_id)
{
...
static fbe_status_t fbe_api_common_init_contiguous_packet_queue_kernel(void)
{
	...
	for (;chunk < FBE_API_PRE_ALLOCATED_PACKETS_CHUNKS; chunk++) {
		...
		/* Allocate the memory */
                ptr = 0;
                ptr = fbe_allocate_contiguous_memory(1048576);
                if (ptr == NULL) {
...
static fbe_status_t fbe_api_common_destroy_contiguous_packet_queue_kernel()
{
	...
	for (;chunk < FBE_API_PRE_ALLOCATED_PACKETS_CHUNKS; chunk++) {
            ...
            fbe_release_contiguous_memory(cmm_chunk_addr[chunk]);
	}
