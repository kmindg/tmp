...
static fbe_status_t fbe_cli_initialize_fbe_api (char driverType , char spId, fbe_u16_t sp_port_base);
static fbe_u32_t fbe_cli_get_target_sp(char spstr);
void __cdecl fbe_cli_destroy_fbe_api(int in);
static void CSX_GX_RT_DEFCC fbe_cli_csx_cleanup_handler(csx_rt_proc_cleanup_context_t context);
...
#include "fbe/fbe_emcutil_shell_maincode.h"
    ...
    if (driverType == 0)
    {
        ...
        printf("\nBy default, driverType is K and spId is a. If you would like to run fbecli.exe on simulation, please use fbecli.exe s a or b\n");
        driverType = 'k';
        spId = 'a';
    }
...
#else
    //csx_rt_proc_cleanup_handler_register(fbe_cli_csx_cleanup_handler, fbe_cli_csx_cleanup_handler, NULL);
#endif
    ...
    status = fbe_cli_initialize_fbe_api(driverType, spId, sp_port_base);
    ...
    if(*scriptFileName != 0)
    {
       ...
       csx_rt_proc_cleanup_handler_deregister(fbe_cli_csx_cleanup_handler);
       fbe_cli_destroy_fbe_api(0);
       is_destroy_needed = FBE_TRUE;
	   return 0;
    }

	if (command_to_run) {
		fbe_cli_run_direct_command(direct_command);
	}else{
		fbe_cli_prompt_mode();
	}
...
static fbe_status_t fbe_cli_initialize_fbe_api (char driverType , char spId, fbe_u16_t sp_port_base)
{
    ...
    if ((driverType == 's') || (driverType == 'S')) {
       ...
       /*need to initialize the client to connect to the server*/
        sp_to_connect = fbe_cli_get_target_sp(spId);
...
void fbe_cli_error(const fbe_char_t * fmt, ...)
{
    ...
    fbe_sprintf(buffer, sizeof(buffer), "FBE-CLI ERROR!  %s", buffer_fmt);
    va_end(argptr);  
...
static void printUsage(void)
{
    printf("Usage: fbecli <driver_type><SP> <output_type><filename>\n");
    ...
    printf("             port_base_number = SP port base for fbecli connects to.  No space\n");
    printf("                        should be used between the sp_port_base and port_base_number.\n");
    printf("Examples:\n");
    printf("  'fbecli s a' - Connect to fbecli running on SP A.\n\n");
    printf("  'fbecli s a o\"C:\\fbe_cli_output.txt\"' - Connect to fbecli running on SP A\n");
    printf("   and log the output to the file \"C:\\fbe_cli_output.txt\".\n");
    printf("  by default, if only run fbecli.exe, it's set to option k a. \n");
    return;
}
