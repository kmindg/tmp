...
static fbe_status_t fbe_cms_lun_access_direct_buffer_io(fbe_payload_block_operation_opcode_t operation,
														fbe_cms_lun_type_t lun_type,
														fbe_lba_t lba,
...
/*this will persist a buffer that already contains the CRC and is guaranteed to be physically contiguous*/
fbe_status_t fbe_cms_lun_access_persist_buffer_direct(fbe_cms_lun_type_t lun_type,
													  ...
													  void *context)
{
	return fbe_cms_lun_access_direct_buffer_io(FBE_PAYLOAD_BLOCK_OPERATION_OPCODE_WRITE,
										lun_type,
										lba,
										block_count,
										sg_list,
...
/*this will read into a buffer that already contains the CRC area and is guaranteed to be physically contiguous*/
fbe_status_t fbe_cms_lun_access_load_buffer_direct(fbe_cms_lun_type_t lun_type,
												   ...
												   void *context)
{
	return fbe_cms_lun_access_direct_buffer_io(FBE_PAYLOAD_BLOCK_OPERATION_OPCODE_READ,
										lun_type,
										lba,
										block_count,
										sg_list,
														...
														void *context)
{
	...
	/*we don't use our own internal memory but since we need to keep the contect + the callbackl function of the user,
	we'll use our pre allocated memory that we ususally use for small IO and use just a small portion of it, this will make
	sure we don't try to allocate on the fly and handle failures*/
	fbe_multicore_queue_lock(&fbe_cms_lun_access_data.small_io_memory_queue_head, cpu_id);
	small_io_memory_ptr = (fbe_cms_lun_access_small_io_memory_t *)fbe_multicore_queue_pop(&fbe_cms_lun_access_data.small_io_memory_queue_head, cpu_id);
	...
	fbe_multicore_queue_unlock(&fbe_cms_lun_access_data.small_io_memory_queue_head, cpu_id);

	io_header_p = &small_io_memory_ptr->header;
    io_header_p->data_type = FBE_CMS_LUN_ACCESS_1_BLOCK_IO;
	io_header_p->completion_func = completion_func;
	io_header_p->context = context;
    ...
    /* Put an sgl pointer in the packet payload. */
    fbe_sep_payload_set_sg_list(payload, sg_list, 2);/*count does not really matter here, we never use it*/

    /* Send send down write. */
    fbe_transport_set_completion_function(packet_p, fbe_cms_lun_access_write_buffer_completion, (void *)io_header_p);
