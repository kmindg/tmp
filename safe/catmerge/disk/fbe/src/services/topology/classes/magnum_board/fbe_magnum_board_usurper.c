...
static fbe_status_t magnum_board_attach_edge(fbe_magnum_board_t * magnum_board, fbe_packet_t * packet);
static fbe_status_t magnum_board_detach_edge(fbe_magnum_board_t * magnum_board, fbe_packet_t * packet);
static fbe_status_t magnum_board_get_port_info(fbe_magnum_board_t * magnum_board, fbe_packet_t * packet);

fbe_status_t 
fbe_magnum_board_control_entry(fbe_object_handle_t object_handle, fbe_packet_t * packet)
{
    fbe_status_t status;
    ...
    switch(control_code) {        
        case FBE_DISCOVERY_TRANSPORT_CONTROL_CODE_ATTACH_EDGE:
            status = magnum_board_attach_edge(magnum_board, packet);
            break;
        case FBE_DISCOVERY_TRANSPORT_CONTROL_CODE_DETACH_EDGE:
            status = magnum_board_detach_edge(magnum_board, packet);
            break;
        case FBE_BASE_BOARD_CONTROL_CODE_GET_PORT_INFO:
            status = magnum_board_get_port_info(magnum_board, packet);
            break;
        default:
...
static fbe_status_t 
magnum_board_attach_edge(fbe_magnum_board_t * magnum_board, fbe_packet_t * packet)
{
    ...
    if (server_index >= FBE_MAGNUM_BOARD_NUMBER_OF_IO_PORTS) {
        fbe_base_object_trace((fbe_base_object_t*)magnum_board, 
                              FBE_TRACE_LEVEL_WARNING,
                              FBE_TRACE_MESSAGE_ID_FUNCTION_FAILED,
    ...
    if (magnum_board->io_port_array[(fbe_u32_t)server_index].io_port_number == FBE_BASE_BOARD_IO_PORT_INVALID) {
        fbe_base_object_trace((fbe_base_object_t*)magnum_board, 
                              FBE_TRACE_LEVEL_WARNING,
                              FBE_TRACE_MESSAGE_ID_FUNCTION_FAILED,
    ...
    fbe_base_discovering_attach_edge((fbe_base_discovering_t*)magnum_board, discovery_transport_control_attach_edge->discovery_edge);
    fbe_base_discovering_set_path_attr((fbe_base_discovering_t*)magnum_board, server_index, 0 /* attr */);
    fbe_base_discovering_update_path_state((fbe_base_discovering_t*)magnum_board);
...
static fbe_status_t 
magnum_board_detach_edge(fbe_magnum_board_t * magnum_board, fbe_packet_t * packet)
{
    fbe_status_t status;
    ...
    status = fbe_base_discovering_detach_edge((fbe_base_discovering_t*)magnum_board,
...
static fbe_status_t
magnum_board_get_port_info(fbe_magnum_board_t * magnum_board, fbe_packet_t * packet)
{
    ...
    server_index = FBE_MAGNUM_BOARD_NUMBER_OF_IO_PORTS;
    ...
    port_info->io_portal_number = (fbe_u32_t)magnum_board->io_port_array[server_index].io_portal_number;
    port_info->io_port_number = (fbe_u32_t)magnum_board->io_port_array[server_index].io_port_number;    
