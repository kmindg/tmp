...
static fbe_queue_head_t     metadata_stripe_lock_blob_queue;
static fbe_spinlock_t       metadata_stripe_lock_blob_queue_lock;

static fbe_u8_t         *   metadata_stripe_lock_peer_sl_memory = NULL; 
static fbe_queue_head_t     metadata_stripe_lock_peer_sl_queue;
static fbe_spinlock_t       metadata_stripe_lock_peer_sl_queue_lock;

static fbe_u64_t metadata_stripe_lock_peer_sl_queue_count = 0; /* This will track alloc and release from peer_sl_queue */

/* Forward declaration */
static fbe_status_t metadata_stripe_lock_lock(fbe_packet_t *  packet);
static fbe_status_t metadata_stripe_lock_unlock(fbe_packet_t * packet);

static fbe_status_t metadata_stripe_lock_start(fbe_packet_t *  packet);
static fbe_status_t metadata_stripe_lock_stop(fbe_packet_t *  packet);


static fbe_status_t fbe_metadata_stripe_lock_control_entry_completion(fbe_packet_t * packet, fbe_packet_completion_context_t context);

static fbe_status_t fbe_metadata_stripe_lock_msg(fbe_metadata_element_t * metadata_element, 
                                                 fbe_metadata_cmi_message_t * metadata_cmi_msg,
                                                 fbe_u32_t cmi_message_length);

static fbe_status_t fbe_metadata_stripe_release_msg(fbe_metadata_element_t * metadata_element, 
                                                 fbe_metadata_cmi_message_t * metadata_cmi_msg,
                                                 fbe_u32_t cmi_message_length);

static fbe_status_t fbe_metadata_stripe_aborted_msg(fbe_metadata_element_t * metadata_element, 
                                                 fbe_metadata_cmi_message_t * metadata_cmi_msg,
                                                 fbe_u32_t cmi_message_length);


static fbe_status_t fbe_metadata_stripe_grant_msg(fbe_metadata_element_t * mde, 
                                                  fbe_metadata_cmi_message_t * msg,
                                                  fbe_u32_t cmi_message_length);

static __forceinline fbe_bool_t metadata_stripe_lock_mark_grant_to_peer(fbe_payload_stripe_lock_operation_t * sl);
static __forceinline fbe_bool_t metadata_stripe_lock_check_peer(fbe_payload_stripe_lock_operation_t * sl);

/* This will send grants to the peer */
static fbe_status_t metadata_stripe_lock_dispatch_cmi_grant_queue(fbe_queue_head_t * grant_queue);


static fbe_status_t metadata_stripe_lock_send_request_sl(fbe_payload_stripe_lock_operation_t * sl, fbe_bool_t * is_cmi_required);

static fbe_status_t fbe_metadata_stripe_send_release_msg(fbe_metadata_element_t * mde, fbe_payload_stripe_lock_operation_t * sl);
static fbe_status_t fbe_metadata_stripe_send_aborted_msg(fbe_metadata_element_t * mde, fbe_payload_stripe_lock_operation_t * sl);

static fbe_status_t metadata_stripe_lock_complete_cancelled_queue(fbe_queue_head_t * queue, fbe_queue_head_t * tmp_queue);

static fbe_status_t metadata_stripe_lock_dispatch_blob(fbe_metadata_stripe_lock_blob_t * blob, 
                                                       fbe_queue_head_t * cmi_request_queue, 
                                                       fbe_queue_head_t * grant_queue);

static fbe_status_t metadata_stripe_lock_dispatch_cmi_request_queue(fbe_queue_head_t * cmi_request_queue);

#define SL_DEBUG_MODE 0

static void metadata_stripe_lock_check_packet_queue(fbe_queue_head_t * queue);
static void metadata_stripe_lock_check_packet(fbe_packet_t * packet);

static fbe_metadata_stripe_lock_hash_table_entry_t * 
fbe_metadata_stripe_hash_get_entry(fbe_metadata_stripe_lock_hash_t * hash, fbe_payload_stripe_lock_operation_t * sl);

static fbe_status_t fbe_metadata_stripe_lock_peer_lost_mde(fbe_metadata_element_t * mde, fbe_queue_head_t * tmp_queue, fbe_bool_t * is_cmi_required);

static fbe_status_t
fbe_metadata_stripe_lock_element_abort_queue(fbe_metadata_element_t * mde, 
                                             fbe_queue_head_t * queue,
                                             fbe_queue_head_t * abort_queue,
                                             fbe_queue_head_t * tmp_queue);

static fbe_status_t 
fbe_stripe_lock_grant_all_peer_locks(fbe_metadata_element_t* mde, fbe_queue_head_t * tmp_queue, fbe_bool_t * is_cmi_required);

static fbe_status_t fbe_stripe_lock_scan_for_grants(fbe_metadata_element_t* mde, fbe_queue_head_t * grant_queue);
static fbe_status_t metadata_stripe_lock_disable_hash(fbe_metadata_element_t * mde);
static fbe_status_t metadata_stripe_lock_enable_hash(fbe_metadata_element_t * mde);
static fbe_status_t metadata_stripe_lock_push_sorted(fbe_queue_head_t * queue, fbe_payload_stripe_lock_operation_t * sl);
static fbe_status_t metadata_stripe_lock_reschedule_blob(fbe_metadata_element_t * mde, fbe_bool_t is_cmi_required, fbe_bool_t is_hash_enable_required);

typedef enum metadata_stripe_lock_thread_flag_e{
    METADATA_STRIPE_LOCK_THREAD_RUN,
    METADATA_STRIPE_LOCK_THREAD_STOP,
    METADATA_STRIPE_LOCK_THREAD_DONE
}metadata_stripe_lock_thread_flag_t;

static fbe_thread_t                         metadata_stripe_lock_thread_handle;
static metadata_stripe_lock_thread_flag_t   metadata_stripe_lock_thread_flag;


static fbe_rendezvous_event_t				metadata_stripe_lock_event;


static __forceinline fbe_status_t 
metadata_stripe_lock_set_region(fbe_payload_stripe_lock_operation_t * sl, fbe_metadata_stripe_lock_region_t * region)
{
...
static __forceinline fbe_status_t 
metadata_stripe_lock_set_invalid_region(fbe_metadata_stripe_lock_region_t * region)
{
...
void fbe_metadata_sl_get_memory_use(fbe_u32_t *memory_bytes_p)
{
...
static __forceinline fbe_status_t 
metadata_stripe_lock_update_region(fbe_payload_stripe_lock_operation_t * sl1, fbe_payload_stripe_lock_operation_t * sl2)
{
...
/* This will update the region of sl2 */
static __forceinline fbe_status_t 
metadata_stripe_lock_update_peer_request_regions(fbe_payload_stripe_lock_operation_t * sl1, fbe_payload_stripe_lock_operation_t * sl2)
{
...
metadata_stripe_lock_is_overlap(fbe_metadata_stripe_lock_region_t * r1, fbe_metadata_stripe_lock_region_t * r2)
{
...
static __forceinline fbe_bool_t 
metadata_stripe_lock_is_np_lock(fbe_payload_stripe_lock_operation_t * sl)
{
    return (sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_NP);
}

static __forceinline fbe_bool_t 
metadata_stripe_lock_is_paged_lock(fbe_payload_stripe_lock_operation_t * sl)
{
    return (sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_PAGED);
}

static __forceinline fbe_bool_t
metadata_stripe_lock_is_monitor_op_abort_needed(fbe_metadata_element_t * mde, fbe_payload_stripe_lock_operation_t * sl, fbe_bool_t abort_peer)
{
    ...
    if (metadata_stripe_lock_is_np_lock(sl)){
        return FBE_FALSE; /* We do not abort non-paged lock */
    }

    if (metadata_stripe_lock_is_paged_lock(sl)){
...
metadata_stripe_lock_get_non_paged_slots(fbe_metadata_element_t * mde,
                                         fbe_u32_t *first_slot,
                                         fbe_u32_t *last_slot)
{
    fbe_metadata_stripe_lock_blob_t * blob = NULL;
...
static __forceinline void 
metadata_stripe_lock_get_paged_slots(fbe_metadata_element_t * mde,
                                     fbe_u32_t *first_slot,
                                     fbe_u32_t *last_slot)
{
    fbe_metadata_stripe_lock_blob_t * blob = NULL;
...
static __forceinline fbe_metadata_lock_slot_state_t 
metadata_stripe_lock_get_slot_state(fbe_metadata_element_t * mde,
                                    fbe_u32_t slot,
                                    fbe_bool_t is_user_slot)
{
    fbe_metadata_stripe_lock_blob_t * blob = mde->stripe_lock_blob;
    fbe_u32_t slot_byte;
    fbe_u32_t sub_slot;

    if (!is_user_slot)
    {
        return (blob->slot[slot] & METADATA_STRIPE_LOCK_SLOT_STATE_MASK);
    }

    /* User slot */
    slot_byte = (slot >> METADATA_STRIPE_LOCK_BITS_PER_SLOT);
    sub_slot = (slot & METADATA_STRIPE_LOCK_SLOT_STATE_MASK);

    return ((blob->slot[slot_byte] >> (sub_slot * METADATA_STRIPE_LOCK_BITS_PER_SLOT)) & METADATA_STRIPE_LOCK_SLOT_STATE_MASK);
}

static __forceinline void 
metadata_stripe_lock_set_slot_state(fbe_metadata_element_t * mde,
                                    fbe_u32_t slot,
                                    fbe_bool_t is_user_slot,
                                    fbe_metadata_lock_slot_state_t set_state)
{
    fbe_metadata_stripe_lock_blob_t * blob = mde->stripe_lock_blob;
    fbe_u32_t slot_word;
    fbe_u32_t sub_slot;
    fbe_u64_t shift;
    fbe_atomic_t mask, state;
    fbe_atomic_t * word_ptr;

    if (!is_user_slot)
    {
        blob->slot[slot] = set_state;
        return;
    }

    /* User slot */
    slot_word = (slot >> (METADATA_STRIPE_LOCK_BITS_PER_SLOT + 3));
    sub_slot = (slot & (METADATA_STRIPE_LOCK_SLOT_STATE_MASK | 0x1c)); /* subslot mask 0x1F, last 5 bits (3 bits for word offset) */
    shift = (fbe_u64_t)sub_slot * METADATA_STRIPE_LOCK_BITS_PER_SLOT;
    mask = ~((fbe_u64_t)METADATA_STRIPE_LOCK_SLOT_STATE_MASK << shift);
    state = ((fbe_u64_t)set_state << shift);

    word_ptr = (fbe_atomic_t *)&(blob->slot[0]);
    word_ptr += slot_word;
    fbe_atomic_and(word_ptr, mask);
    fbe_atomic_or(word_ptr, state);

#if 0
    if (fbe_base_config_metadata_element_to_object_id(mde) > 0x100) {
        metadata_trace(FBE_TRACE_LEVEL_INFO,
                       FBE_TRACE_MESSAGE_ID_INFO,
                       "SL: obj: 0x%x word %d subslot %d shift 0x%llx, mask 0x%llx state 0x%llx\n",
                       fbe_base_config_metadata_element_to_object_id(mde), slot_word, sub_slot, shift, mask, state);
        metadata_trace(FBE_TRACE_LEVEL_INFO,
                       FBE_TRACE_MESSAGE_ID_INFO,
                       "SL: obj: 0x%x slot 0x%x state %x word 0x%llx ptr %p\n",
                       fbe_base_config_metadata_element_to_object_id(mde), slot, set_state,  *word_ptr, word_ptr);
    }
#endif
...
static __forceinline void 
metadata_stripe_lock_get_lock_slots(fbe_payload_stripe_lock_operation_t * sl,
                                    fbe_u32_t *first_slot,
                                    fbe_u32_t *last_slot)
{
    fbe_metadata_element_t * mde = NULL;
    ...
    {
        metadata_stripe_lock_get_non_paged_slots(mde,first_slot,last_slot);
    }
    else
    {
        if (sl->stripe.first >= blob->private_slot)
        { /* Paged lock */
            metadata_stripe_lock_get_paged_slots(mde,first_slot,last_slot);
        }
        else
        {
            /* It is user lock */
...
static __forceinline fbe_bool_t 
metadata_stripe_lock_check_sorted(fbe_payload_stripe_lock_operation_t * sl1, 
...
static __forceinline void 
metadata_stripe_lock_set_read_overlap(fbe_payload_stripe_lock_operation_t * sl1, 
                                    fbe_payload_stripe_lock_operation_t * sl2,
                                    fbe_bool_t * read_overlap)
{
    ...
    */
            if((sl1->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST) && (sl2->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST)){
                metadata_stripe_lock_update_peer_request_regions(sl2, sl1);
            }
#endif
...
/* The order is IMPORTANT */
static fbe_bool_t 
metadata_stripe_lock_check_overlap(fbe_payload_stripe_lock_operation_t * sl1, 
                                    fbe_payload_stripe_lock_operation_t * sl2,
                                    fbe_bool_t * read_overlap,
                                    fbe_bool_t * reinsert_requiered)
{
    ...
    if(sl1->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_READ){ /* sl1 is a read */
        ...
        if(sl1->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST){
            if(metadata_stripe_lock_is_overlap(&sl1->cmi_stripe_lock.read_region, &sl2->cmi_stripe_lock.write_region)){
                /* Peer requests can not overlap */
                if((sl1->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST) && (sl2->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST)){
                    /* Make sure it is sorted */
                    metadata_stripe_lock_set_read_overlap(sl1, sl2, read_overlap);
            ...
            if(metadata_stripe_lock_is_overlap(&sl1->cmi_stripe_lock.read_region, &sl2->cmi_stripe_lock.read_region)){
                if(sl1->cmi_stripe_lock.read_region.last > sl2->cmi_stripe_lock.read_region.last){
                    *read_overlap = FBE_TRUE;
                }
            }
        ...
        if(metadata_stripe_lock_is_overlap(&sl1->stripe, &sl2->cmi_stripe_lock.write_region)){
            return FBE_TRUE;
        }
        if(metadata_stripe_lock_is_overlap(&sl1->cmi_stripe_lock.read_region, &sl2->cmi_stripe_lock.read_region)){
            if(sl1->cmi_stripe_lock.read_region.last > sl2->cmi_stripe_lock.read_region.last){
        ...
        /* For the peer request check the region first */
        if(sl1->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST){
            if(metadata_stripe_lock_is_overlap(&sl1->cmi_stripe_lock.write_region, &sl2->cmi_stripe_lock.write_region) ||
                metadata_stripe_lock_is_overlap(&sl1->cmi_stripe_lock.write_region, &sl2->cmi_stripe_lock.read_region)){
        ...
        if(metadata_stripe_lock_is_overlap(&sl1->stripe, &sl2->cmi_stripe_lock.read_region)){
            return FBE_TRUE;
        }
    }
...
static void 
metadata_stripe_lock_thread_func(void * context)
{
    fbe_queue_head_t grant_queue;
    fbe_queue_head_t cmi_request_queue;
    ...
    while(1) {
        fbe_rendezvous_event_wait(&metadata_stripe_lock_event, 0);
        if(metadata_stripe_lock_thread_flag == METADATA_STRIPE_LOCK_THREAD_RUN) {
            fbe_spinlock_lock(&metadata_stripe_lock_blob_queue_lock);
            while(queue_element = fbe_queue_pop(&metadata_stripe_lock_blob_queue)){
                metadata_stripe_lock_dispatch_blob((fbe_metadata_stripe_lock_blob_t *)queue_element, &cmi_request_queue, &grant_queue); 
            }
            fbe_rendezvous_event_clear(&metadata_stripe_lock_event);
            fbe_spinlock_unlock(&metadata_stripe_lock_blob_queue_lock);			

            metadata_stripe_lock_dispatch_cmi_grant_queue(&grant_queue);
            metadata_stripe_lock_dispatch_cmi_request_queue(&cmi_request_queue);
        } else {
            break;
        }
    }/* while(1) */
    ...
    metadata_stripe_lock_thread_flag = METADATA_STRIPE_LOCK_THREAD_DONE;
    fbe_thread_exit(EMCPAL_STATUS_SUCCESS);
}

fbe_status_t 
fbe_metadata_stripe_lock_init(void)
{
    fbe_payload_stripe_lock_operation_t * sl;
    fbe_u32_t i;
    fbe_cpu_id_t cpu_count;

    fbe_queue_init(&metadata_stripe_lock_blob_queue);
    fbe_spinlock_init(&metadata_stripe_lock_blob_queue_lock);
    ...
    //metadata_stripe_lock_peer_sl_memory = fbe_memory_native_allocate(sizeof(fbe_payload_stripe_lock_operation_t) * METADATA_STRIPE_LOCK_MAX_PEER_SL);
    metadata_stripe_lock_peer_sl_memory = 
        fbe_memory_allocate_required(sizeof(fbe_payload_stripe_lock_operation_t) * METADATA_STRIPE_LOCK_MAX_PEER_SL * cpu_count);
    
    fbe_queue_init(&metadata_stripe_lock_peer_sl_queue);
    fbe_spinlock_init(&metadata_stripe_lock_peer_sl_queue_lock);

    sl = (fbe_payload_stripe_lock_operation_t *)metadata_stripe_lock_peer_sl_memory;
    for(i = 0; i < (METADATA_STRIPE_LOCK_MAX_PEER_SL  * cpu_count); i++){
        fbe_queue_element_init(&sl->queue_element);
        fbe_queue_push(&metadata_stripe_lock_peer_sl_queue, &sl->queue_element);
        sl++;
    }

    
    
    fbe_rendezvous_event_init(&metadata_stripe_lock_event);

    /* Start thread */
    metadata_stripe_lock_thread_flag = METADATA_STRIPE_LOCK_THREAD_RUN;
    fbe_thread_init(&metadata_stripe_lock_thread_handle, "fbe_meta_strlock", metadata_stripe_lock_thread_func, (void*)(fbe_ptrhld_t)i);

    return FBE_STATUS_OK;
}

fbe_status_t 
fbe_metadata_stripe_lock_destroy(void)
{
    fbe_cpu_id_t cpu_count;

    cpu_count = fbe_get_cpu_count();
    metadata_stripe_lock_thread_flag = METADATA_STRIPE_LOCK_THREAD_STOP;  
    fbe_rendezvous_event_set(&metadata_stripe_lock_event);
    fbe_thread_wait(&metadata_stripe_lock_thread_handle);
    fbe_thread_destroy(&metadata_stripe_lock_thread_handle);

    /* Check if we get all Sl released to the queue */
    if(metadata_stripe_lock_peer_sl_queue_count != 0){
        metadata_trace(FBE_TRACE_LEVEL_WARNING,
                       FBE_TRACE_MESSAGE_ID_INFO,
                       "SL: Destroy - Invalid metadata_stripe_lock_peer_sl_queue_count %lld \n", metadata_stripe_lock_peer_sl_queue_count);
    }

    if(fbe_queue_length(&metadata_stripe_lock_peer_sl_queue) != (METADATA_STRIPE_LOCK_MAX_PEER_SL*cpu_count)){
        metadata_trace(FBE_TRACE_LEVEL_WARNING,
                       FBE_TRACE_MESSAGE_ID_INFO,
                       "SL: Destroy - Invalid queue length %I64d \n", fbe_queue_length(&metadata_stripe_lock_peer_sl_queue));
    }
    
    fbe_rendezvous_event_destroy(&metadata_stripe_lock_event);

    fbe_queue_destroy(&metadata_stripe_lock_blob_queue);
    fbe_spinlock_destroy(&metadata_stripe_lock_blob_queue_lock);

    fbe_memory_release_required(metadata_stripe_lock_peer_sl_memory);

    fbe_queue_destroy(&metadata_stripe_lock_peer_sl_queue);
    fbe_spinlock_destroy(&metadata_stripe_lock_peer_sl_queue_lock);

    return FBE_STATUS_OK;
}

fbe_status_t 
fbe_stripe_lock_operation_entry(fbe_packet_t *  packet)
{
    fbe_status_t status;
    ...
    case FBE_PAYLOAD_STRIPE_LOCK_OPERATION_OPCODE_READ_LOCK:
        ...
        status = metadata_stripe_lock_lock(packet);
        break;
    ...
    case FBE_PAYLOAD_STRIPE_LOCK_OPERATION_OPCODE_READ_UNLOCK:
        ...
        status = metadata_stripe_lock_unlock(packet);
        break;

    case FBE_PAYLOAD_STRIPE_LOCK_OPERATION_OPCODE_START:
        status = metadata_stripe_lock_start(packet);
        break;

    case FBE_PAYLOAD_STRIPE_LOCK_OPERATION_OPCODE_STOP:
        status = metadata_stripe_lock_stop(packet);
        break;
...
#if 0
static __forceinline void
metadata_stripe_lock_update_write_count(fbe_payload_stripe_lock_operation_t * sl)
{
    fbe_metadata_element_t * mde = NULL;
    fbe_metadata_stripe_lock_blob_t * blob = NULL;
...
static fbe_bool_t 
metadata_stripe_lock_check_queue_overlap(fbe_payload_stripe_lock_operation_t * sl, fbe_queue_head_t * queue)
{
    fbe_queue_element_t * queue_element = NULL;
    ...
    while(queue_element != NULL){ 
        ...
        if(metadata_stripe_lock_check_overlap(sl, current_sl, &read_overlap, &reinsert_requiered)){ 
            metadata_trace(FBE_TRACE_LEVEL_CRITICAL_ERROR,
                            FBE_TRACE_MESSAGE_ID_INFO,
...
static fbe_bool_t 
metadata_stripe_lock_check_queue_overlap_with_peer(fbe_queue_head_t * queue)
{
    fbe_queue_element_t * queue_element = NULL;
    fbe_payload_stripe_lock_operation_t * current_sl = NULL;
    ...
    while(queue_element != NULL){ 
        ...
        if(!(current_sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST)  &&
            ...
            !metadata_stripe_lock_check_peer(current_sl)){ 
            metadata_trace(FBE_TRACE_LEVEL_CRITICAL_ERROR,
                            FBE_TRACE_MESSAGE_ID_INFO,
...
static fbe_bool_t
metadata_stripe_lock_insert(fbe_payload_stripe_lock_operation_t * sl, fbe_queue_head_t * q)
{
    fbe_queue_element_t * queue_element = NULL;
    fbe_payload_stripe_lock_operation_t * current_sl = NULL;
    ...
    if((mde->stripe_lock_hash != NULL) && !(mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_SL_HASH_DISABLED)){
        fbe_metadata_stripe_lock_hash_table_entry_t * he;
        he = fbe_metadata_stripe_hash_get_entry(mde->stripe_lock_hash, sl);
        queue = &he->head;
    } else {
        queue = q;
    }
    ...
    if(!fbe_queue_is_empty(&sl->wait_queue)){
        ...
        if(!(sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST)){
            if (sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_READ) {
                metadata_stripe_lock_set_region(sl, &sl->cmi_stripe_lock.read_region);
                metadata_stripe_lock_set_invalid_region(&sl->cmi_stripe_lock.write_region);
            } else { /* write_lock */
                metadata_stripe_lock_set_region(sl, &sl->cmi_stripe_lock.write_region);
                metadata_stripe_lock_set_invalid_region(&sl->cmi_stripe_lock.read_region);
            }
        }
    }
    ...
    while(queue_element != NULL){ 
        ...
        if(metadata_stripe_lock_check_sorted(sl, current_sl)){ /* Order is IMPORTANT */
            // Sanity testing
            //metadata_stripe_lock_check_queue_overlap(sl, queue);
            ...
            if(is_deadlock == FBE_TRUE){
                while(wait_element = fbe_queue_pop(&deadlock_queue)){ 
                    ...
                    metadata_stripe_lock_update_region(sl, tmp_sl);
                }
            }
        ...
        if(metadata_stripe_lock_check_overlap(sl, current_sl, &read_overlap, &reinsert_requiered)){ /* The order of arguments is IMPORTANT */
            ...
            //metadata_stripe_lock_update_region(sl, current_sl);
            ...
            if((sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST) &&
                ...
                if (current_sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_READ) {
                    metadata_stripe_lock_set_region(current_sl, &current_sl->cmi_stripe_lock.read_region);
                    metadata_stripe_lock_set_invalid_region(&current_sl->cmi_stripe_lock.write_region);
                } else { /* write_lock */
                    metadata_stripe_lock_set_region(current_sl, &current_sl->cmi_stripe_lock.write_region);
                    metadata_stripe_lock_set_invalid_region(&current_sl->cmi_stripe_lock.read_region);
                }
            ...
            if(sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST && !metadata_common_cmi_is_active()){
                ...
                if(is_deadlock == FBE_TRUE){
                    ...
                    while(wait_element = fbe_queue_pop(&deadlock_queue)){ 
                        // We need to update the region 
                        tmp_sl = fbe_metadata_stripe_lock_queue_element_to_operation(wait_element);
                        metadata_stripe_lock_update_region(tmp_sl, current_sl);
    ...
    if(is_deadlock == FBE_TRUE){
        while(wait_element = fbe_queue_pop(&deadlock_queue)){ 
            ...
            tmp_sl = fbe_metadata_stripe_lock_queue_element_to_operation(wait_element);
            metadata_stripe_lock_update_region(tmp_sl, sl);
        }
    }
...
static __forceinline fbe_bool_t 
metadata_stripe_lock_check_peer(fbe_payload_stripe_lock_operation_t * sl)
{	
    fbe_u32_t first_slot;
    fbe_u32_t last_slot;
    fbe_metadata_element_t * mde = NULL;
    fbe_metadata_stripe_lock_blob_t * blob = NULL;
    fbe_u32_t i;
    fbe_bool_t is_user_slot = FBE_FALSE;
    ...
    if(sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_READ){
        for(i = first_slot; i <= last_slot; i++){
            if(metadata_stripe_lock_get_slot_state(mde, i, is_user_slot) == METADATA_STRIPE_LOCK_SLOT_STATE_EXCLUSIVE_PEER){
                /* We have collision */
        ...
        for(i = first_slot; i <= last_slot; i++){
            if(metadata_stripe_lock_get_slot_state(mde, i, is_user_slot) != METADATA_STRIPE_LOCK_SLOT_STATE_EXCLUSIVE_LOCAL){
                /* We have collision */
                /* Mark sl with peer collision*/
                sl->flags |= FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_COLLISION;
...
/* Check peer collision just before the lock is granted */
static __forceinline fbe_bool_t 
metadata_stripe_lock_mark_grant_to_peer(fbe_payload_stripe_lock_operation_t * sl)
{	
    fbe_u32_t first_slot;
    fbe_u32_t last_slot;
    fbe_metadata_element_t * mde = NULL;
    ...
    if(sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_READ){
        for(i = first_slot; i <= last_slot; i++){
            if(metadata_stripe_lock_get_slot_state(mde, i, is_user_slot) == METADATA_STRIPE_LOCK_SLOT_STATE_EXCLUSIVE_LOCAL){
                metadata_stripe_lock_set_slot_state(mde, i, is_user_slot, METADATA_STRIPE_LOCK_SLOT_STATE_SHARED);
            }
        }
    } else { /* It is write */
        for(i = first_slot; i <= last_slot; i++){
            metadata_stripe_lock_set_slot_state(mde, i, is_user_slot, METADATA_STRIPE_LOCK_SLOT_STATE_EXCLUSIVE_PEER);
        }
    }
    ...
    // Sanity testing
    //metadata_stripe_lock_check_queue_overlap_with_peer(&mde->stripe_lock_queue_head);
...
static __forceinline fbe_bool_t 
metadata_stripe_lock_is_cmi_required(fbe_payload_stripe_lock_operation_t * sl)
{
    fbe_metadata_element_t * mde = NULL;
...
static __forceinline fbe_bool_t 
metadata_stripe_lock_mark_grant_from_peer(fbe_metadata_element_t * mde, 
...
{	
    ...
    if(cmi_stripe_lock->header.metadata_cmi_message_type == FBE_METADATA_CMI_MESSAGE_TYPE_STRIPE_READ_GRANT){
        ...
        for(i = first_slot; i <= last_slot; i++){
            metadata_stripe_lock_set_slot_state(mde, i, is_user_slot, METADATA_STRIPE_LOCK_SLOT_STATE_EXCLUSIVE_LOCAL);
        }
    }
...
static __forceinline fbe_status_t
metadata_stripe_lock_dispatch(fbe_payload_stripe_lock_operation_t * sl, fbe_queue_head_t * tmp_queue, fbe_bool_t * is_cmi_required)
{
    fbe_queue_element_t * queue_element = NULL;
    fbe_payload_stripe_lock_operation_t * current_sl = NULL;
    fbe_metadata_element_t * mde = NULL;
    ...
    while(queue_element = fbe_queue_pop(&sl->wait_queue)){ 
        ...
        if(!(current_sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST) && /* if it is not peer request */
            ...
            !(current_sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_WAITING_FOR_PEER) && /* We can not cancel this packet */
                ...
                (!metadata_stripe_lock_is_np_lock(sl) && (mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_ABORT_STRIPE_LOCKS)) ||
                ((mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_ABORT_MONITOR_OPS) && metadata_stripe_lock_is_monitor_op_abort_needed(mde, current_sl, FBE_FALSE))))
        {
            if(current_sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_LARGE){
                fbe_atomic_decrement(&mde->stripe_lock_large_io_count);
            }
            ...
            if(fbe_transport_is_packet_cancelled(current_sl->cmi_stripe_lock.packet)){
                ...
                fbe_transport_record_callback_with_action(current_sl->cmi_stripe_lock.packet,
                                                            (fbe_packet_completion_function_t) metadata_stripe_lock_dispatch, 
                                                            PACKET_SL_ABORT);
        ...
        if(metadata_stripe_lock_insert(current_sl, &mde->stripe_lock_queue_head)){ /* We done: Stripe lock granted */
            if(current_sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST){
                metadata_stripe_lock_mark_grant_to_peer(current_sl); /* Mark slots */
                //fbe_queue_push(grant_queue, (fbe_queue_element_t *)current_sl); /* This is ugly */
                * is_cmi_required = FBE_TRUE;
                continue;
            }
            if(metadata_stripe_lock_check_peer(current_sl)){ /* No collision with peer */
                ...
                metadata_stripe_lock_send_request_sl(current_sl, is_cmi_required);
            }
        }
    }
...
static fbe_status_t
metadata_stripe_lock_send_request_sl(fbe_payload_stripe_lock_operation_t * sl, fbe_bool_t * is_cmi_required)
{
    ...
    if(!metadata_stripe_lock_is_cmi_required(sl)){
        return FBE_STATUS_OK;
    }
    ...
    /* Allocate request_sl */
    fbe_spinlock_lock(&metadata_stripe_lock_peer_sl_queue_lock);
    queue_element = fbe_queue_pop(&metadata_stripe_lock_peer_sl_queue);	
    metadata_stripe_lock_peer_sl_queue_count++;
    fbe_spinlock_unlock(&metadata_stripe_lock_peer_sl_queue_lock);
...
static fbe_status_t 
metadata_stripe_lock_dispatch_cmi_request_queue(fbe_queue_head_t * cmi_request_queue)
{
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_metadata_element_t * mde = NULL;
    fbe_queue_element_t * queue_element  = NULL;
...
static fbe_status_t 
metadata_stripe_lock_dispatch_cmi_grant_queue(fbe_queue_head_t * grant_queue)
{
        ...
        if(sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_READ){
            ...
            if(metadata_stripe_lock_is_paged_lock(sl)){
                if (sl->cmi_stripe_lock.flags & FBE_METADATA_CMI_STRIPE_LOCK_FLAG_NEED_RELEASE) {
                    ...
                    /* Invalidate the whole paged area */
...
static fbe_status_t 
metadata_stripe_lock_dispatch_blob(fbe_metadata_stripe_lock_blob_t * blob, fbe_queue_head_t * cmi_request_queue, fbe_queue_head_t * grant_queue)
{
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    ...
    while(queue_element = fbe_queue_pop(&tmp_queue)){
        ...
        if(sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST){
            ...
            /* Check if the gate has been closed. */
                       ...
                       (!metadata_stripe_lock_is_np_lock(sl) && (mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_ABORT_STRIPE_LOCKS)) ||
                       ((mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_ABORT_MONITOR_OPS) && metadata_stripe_lock_is_monitor_op_abort_needed(mde, sl, FBE_FALSE))) {
                ...
                fbe_metadata_stripe_send_aborted_msg(mde, sl);
            } else {
                ...
                if(mde->stripe_lock_hash != NULL){
                    if(((sl->stripe.first / mde->stripe_lock_hash->divisor) != (sl->stripe.last / mde->stripe_lock_hash->divisor)) &&
                        (!metadata_stripe_lock_is_np_lock(sl) && !metadata_stripe_lock_is_paged_lock(sl)) ){
                            fbe_metadata_stripe_lock_all(mde);
                            if(!(mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_SL_HASH_DISABLED)) {
                                fbe_spinlock_lock(&mde->stripe_lock_spinlock);

                                metadata_stripe_lock_disable_hash(mde);
                            }
                            ...
                            fbe_metadata_stripe_unlock_all(mde);
                    }
                }

                fbe_metadata_stripe_lock_hash(mde, sl);
                if(metadata_stripe_lock_insert(sl, &mde->stripe_lock_queue_head)){ /* We done: Stripe lock granted */					
                    metadata_stripe_lock_mark_grant_to_peer(sl); /* Mark slots */
                }
    ...
    if(mde->stripe_lock_blob->flags & METADATA_STRIPE_LOCK_BLOB_FLAG_ENABLE_HASH){
        ...
        if(mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_SL_HASH_DISABLED) {
            ...
            metadata_stripe_lock_enable_hash(mde);
...
static fbe_status_t 
metadata_stripe_lock_lock(fbe_packet_t *  packet)
{   
    fbe_payload_ex_t * sep_payload = NULL;
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_metadata_element_t * mde = NULL;
    ...
    if ((mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_DESTROY_ABORT_STRIPE_LOCKS) ||
        (!metadata_stripe_lock_is_np_lock(sl) &&
         ...
         !metadata_stripe_lock_is_np_lock(sl) && !metadata_stripe_lock_is_paged_lock(sl)))
    {
        ...
        //fbe_spinlock_unlock(&mde->stripe_lock_spinlock);
        fbe_metadata_stripe_unlock_hash(mde, sl);
        fbe_transport_record_callback_with_action(packet, (fbe_packet_completion_function_t) metadata_stripe_lock_lock, PACKET_SL_ABORT);
        fbe_payload_stripe_lock_set_status(sl, FBE_PAYLOAD_STRIPE_LOCK_STATUS_ABORTED);
        fbe_transport_set_status(packet, FBE_STATUS_OK, 0);
    ...
    //metadata_stripe_lock_update_write_count(sl);
    fbe_payload_stripe_lock_set_status(sl, FBE_PAYLOAD_STRIPE_LOCK_STATUS_PENDING);

    if(metadata_stripe_lock_insert(sl, &mde->stripe_lock_queue_head)){ /* We done: Stripe lock granted */
        if(metadata_stripe_lock_check_peer(sl)){ /* No collision with peer */
            fbe_payload_stripe_lock_set_status(sl, FBE_PAYLOAD_STRIPE_LOCK_STATUS_OK);
            fbe_transport_set_status(packet, FBE_STATUS_OK, 0);
            //fbe_spinlock_unlock(&mde->stripe_lock_spinlock);
            fbe_metadata_stripe_unlock_hash(mde, sl);
            ...
            metadata_stripe_lock_reschedule_blob(mde, is_cmi_required, is_hash_enable_required);

            return FBE_STATUS_OK;
        } else {
            fbe_transport_set_cancel_function(packet, fbe_metadata_cancel_function, packet);
            /* Send CMI message */
            metadata_stripe_lock_send_request_sl(sl, &is_cmi_required);
        }
    }
...
static fbe_status_t 
metadata_stripe_lock_unlock(fbe_packet_t *  packet)
{    
    fbe_payload_ex_t * sep_payload = NULL;
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_metadata_element_t * mde = NULL;
    ...
    // Sanity testing
    // if(!metadata_stripe_lock_check_peer(sl)){ fbe_debug_break(); }
    ...
    fbe_payload_stripe_lock_set_status(sl, FBE_PAYLOAD_STRIPE_LOCK_STATUS_OK);
    metadata_stripe_lock_dispatch(sl, &tmp_queue, &is_cmi_required); 
    ...
    metadata_stripe_lock_reschedule_blob(mde, is_cmi_required, FBE_FALSE);
    ...
    if(sl->cmi_stripe_lock.flags & FBE_METADATA_CMI_STRIPE_LOCK_FLAG_NEED_RELEASE){
        ...
        fbe_metadata_stripe_send_release_msg(mde, sl);
        return FBE_STATUS_OK;
    }
...
static fbe_status_t
fbe_metadata_stripe_lock_abort_paged_waiters(fbe_queue_head_t * wait_queue, fbe_queue_head_t * abort_queue)
{
    fbe_queue_element_t * queue_element = NULL;
    fbe_payload_stripe_lock_operation_t * sl = NULL;
...
{
    if ((mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_ABORT_STRIPE_LOCKS) ||
        (mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_DESTROY_ABORT_STRIPE_LOCKS) ||
        (mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_ABORT_STRIPE_LOCKS) ||
        (mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_FAIL_PAGED)) {
        return FBE_TRUE;
    } else {
        return FBE_FALSE;
    }
...
static fbe_status_t
fbe_metadata_stripe_lock_element_abort_impl(fbe_metadata_element_t * mde,
...
{    
    ...
    if((mde->stripe_lock_hash != NULL) && !(mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_SL_HASH_DISABLED)){
        for(i = 0; i < mde->stripe_lock_hash->table_size; i++){
            fbe_metadata_stripe_lock_element_abort_queue(mde, &mde->stripe_lock_hash->table[i].head, &abort_queue, &tmp_queue);
        }
    } else {
        fbe_metadata_stripe_lock_element_abort_queue(mde,&mde->stripe_lock_queue_head, &abort_queue, &tmp_queue);
    }
    ...
    while(abort_element = fbe_queue_pop(&abort_queue)){
        ...
        if((!abort_non_paged && metadata_stripe_lock_is_np_lock(abort_sl)) || 
            ...
            if(abort_sl->priv_flags & FBE_PAYLOAD_STRIPE_LOCK_PRIV_FLAG_READ){
                metadata_stripe_lock_set_region(abort_sl, &abort_sl->cmi_stripe_lock.read_region);
                metadata_stripe_lock_set_invalid_region(&abort_sl->cmi_stripe_lock.write_region);
            } else { /* write_lock */
                metadata_stripe_lock_set_region(abort_sl, &abort_sl->cmi_stripe_lock.write_region);
                metadata_stripe_lock_set_invalid_region(&abort_sl->cmi_stripe_lock.read_region);
            }

            if(metadata_stripe_lock_insert(abort_sl, &mde->stripe_lock_queue_head)){ /* We done: Stripe lock granted */
                if(abort_sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST){
                    metadata_stripe_lock_mark_grant_to_peer(abort_sl); /* Mark slots */
                    //fbe_queue_push(grant_queue, (fbe_queue_element_t *)sl); /* This is ugly */
                    is_cmi_required = FBE_TRUE;
                    continue;
                }
                if(metadata_stripe_lock_check_peer(abort_sl)){ /* No collision with peer */
                    ...
                    metadata_stripe_lock_send_request_sl(abort_sl, &is_cmi_required);
                }
            }
        } else {
            ...
            if((abort_sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST) && 
                ...
                //metadata_stripe_lock_update_write_count(abort_sl);

                fbe_metadata_stripe_send_aborted_msg(mde, abort_sl);
                continue;
            }

            if(abort_sl->cmi_stripe_lock.flags & FBE_METADATA_CMI_STRIPE_LOCK_FLAG_NEED_RELEASE){
                fbe_transport_record_callback_with_action(abort_sl->cmi_stripe_lock.packet, (fbe_packet_completion_function_t) fbe_metadata_stripe_lock_element_abort_impl, PACKET_SL_ABORT);
                fbe_payload_stripe_lock_set_status(abort_sl, FBE_PAYLOAD_STRIPE_LOCK_STATUS_ABORTED);
                //metadata_stripe_lock_update_write_count(abort_sl);
                /* need to set packet status here, otherwise, fbe_provision_drive_utils_process_block_status() will complain. */
                fbe_transport_set_status(abort_sl->cmi_stripe_lock.packet, FBE_STATUS_OK, 0);

               if(metadata_stripe_lock_is_paged_lock(abort_sl)){
                    fbe_metadata_paged_release_blobs(mde, abort_sl->stripe.first, abort_sl->stripe.last - abort_sl->stripe.first + 1);
                }

                fbe_metadata_stripe_send_release_msg(mde, abort_sl);
                continue;
            }
...
fbe_status_t 
fbe_metadata_stripe_lock_element_abort(fbe_metadata_element_t * mde)
{
    // Abort all stripe locks except non paged.
    return fbe_metadata_stripe_lock_element_abort_impl(mde, FALSE);
}


fbe_status_t
fbe_metadata_stripe_lock_element_destroy_abort(fbe_metadata_element_t * mde)
{
    // Abort all stripe locks including non paged.
    return fbe_metadata_stripe_lock_element_abort_impl(mde, TRUE);
}


static fbe_status_t 
fbe_metadata_stripe_lock_element_abort_monitor_op_in_queue(fbe_metadata_element_t * mde, 
                                                           ...
                                                           fbe_bool_t * is_cmi_required)
{
    ...
    while(queue_element != NULL){
        sl = fbe_metadata_stripe_lock_queue_element_to_operation(queue_element);
        if ((sl->status == FBE_PAYLOAD_STRIPE_LOCK_STATUS_PENDING) && metadata_stripe_lock_is_monitor_op_abort_needed(mde, sl, abort_peer))
        {
        ...
        else
        {
            ...
            while (wait_q_element)
            {
                ...
                if (metadata_stripe_lock_is_monitor_op_abort_needed(mde, wait_q_sl, abort_peer))
                {
...
fbe_status_t 
fbe_metadata_stripe_lock_element_abort_monitor_ops(fbe_metadata_element_t * mde, fbe_bool_t abort_peer)
{
    fbe_queue_head_t tmp_queue;    
    fbe_payload_stripe_lock_operation_t * abort_sl = NULL;
    fbe_bool_t is_cmi_required = FBE_FALSE;
    ...
    if (fbe_base_config_metadata_element_to_class_id(mde) == FBE_CLASS_ID_EXTENT_POOL) {
        /* Abort monitor ops for extent pool is not enabled now */
        //fbe_ext_pool_lock_element_abort_monitor_ops(mde, abort_peer);
        return FBE_STATUS_OK;
    }
    ...
    if((mde->stripe_lock_hash != NULL) && !(mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_SL_HASH_DISABLED)){
        for(i = 0; i < mde->stripe_lock_hash->table_size; i++){
            fbe_metadata_stripe_lock_element_abort_monitor_op_in_queue(mde, &mde->stripe_lock_hash->table[i].head, abort_peer, &abort_sl, &tmp_queue, &is_cmi_required);
            if (abort_sl != NULL) {
                break;
            }
        }
    } else {
        fbe_metadata_stripe_lock_element_abort_monitor_op_in_queue(mde, &mde->stripe_lock_queue_head, abort_peer, &abort_sl, &tmp_queue, &is_cmi_required);
    }
    ...
    if (abort_sl){
        ...
        if (!abort_peer) {
            fbe_transport_record_callback_with_action(abort_sl->cmi_stripe_lock.packet, (fbe_packet_completion_function_t)fbe_metadata_stripe_lock_element_abort_monitor_ops, PACKET_SL_ABORT);
            fbe_payload_stripe_lock_set_status(abort_sl, FBE_PAYLOAD_STRIPE_LOCK_STATUS_ABORTED);
            fbe_transport_set_status(abort_sl->cmi_stripe_lock.packet, FBE_STATUS_OK, 0);
            if (fbe_queue_is_element_on_queue(&abort_sl->cmi_stripe_lock.packet->queue_element)){
                metadata_trace(FBE_TRACE_LEVEL_CRITICAL_ERROR, 
...
fbe_status_t 
metadata_stripe_lock_update(fbe_metadata_element_t * mde)
{
    fbe_metadata_stripe_lock_blob_t * blob = NULL;
    fbe_u32_t blob_size;
    blob = mde->stripe_lock_blob;
...
static fbe_status_t 
metadata_stripe_lock_start(fbe_packet_t *  packet)
{   
    fbe_payload_ex_t * sep_payload = NULL;
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_metadata_element_t * mde = NULL;
    fbe_metadata_stripe_lock_blob_t * blob = NULL;
    fbe_u8_t el_byte = 0, ep_byte = 0;
    fbe_u32_t i;
     ...
     */
    metadata_stripe_lock_update(mde);
...
static fbe_status_t 
metadata_stripe_lock_stop(fbe_packet_t *  packet)
{   
    fbe_payload_ex_t * sep_payload = NULL;
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_metadata_element_t * mde = NULL;
...
fbe_status_t 
fbe_metadata_stripe_lock_cmi_dispatch(fbe_metadata_element_t * metadata_element, 
                                      fbe_metadata_cmi_message_t * metadata_cmi_msg,
                                      fbe_u32_t cmi_message_length)
{

    if (fbe_base_config_metadata_element_to_class_id(metadata_element) == FBE_CLASS_ID_EXTENT_POOL) {
        return fbe_ext_pool_lock_cmi_dispatch(metadata_element, metadata_cmi_msg, cmi_message_length);
    }

    switch(metadata_cmi_msg->header.metadata_cmi_message_type){
        case FBE_METADATA_CMI_MESSAGE_TYPE_STRIPE_LOCK_START:
            if(!fbe_metadata_stripe_lock_is_started(metadata_element)){
                //metadata_stripe_lock_allocate_blob(metadata_element);
            }
            break;
        ...
        case FBE_METADATA_CMI_MESSAGE_TYPE_STRIPE_WRITE_LOCK:
            fbe_metadata_stripe_lock_msg(metadata_element, metadata_cmi_msg, cmi_message_length);
            break;

        case FBE_METADATA_CMI_MESSAGE_TYPE_STRIPE_RELEASE:
            fbe_metadata_stripe_release_msg(metadata_element, metadata_cmi_msg, cmi_message_length);
            break;

        case FBE_METADATA_CMI_MESSAGE_TYPE_STRIPE_ABORTED:
            fbe_metadata_stripe_aborted_msg(metadata_element, metadata_cmi_msg, cmi_message_length);
            break;

        case FBE_METADATA_CMI_MESSAGE_TYPE_STRIPE_READ_GRANT:
        case FBE_METADATA_CMI_MESSAGE_TYPE_STRIPE_WRITE_GRANT:
            fbe_metadata_stripe_grant_msg(metadata_element, metadata_cmi_msg, cmi_message_length);	
            break;
        default:
...
fbe_bool_t 
fbe_metadata_stripe_lock_is_started(fbe_metadata_element_t * mde)
{
...
fbe_status_t 
fbe_metadata_stripe_lock_control_entry(fbe_packet_t * packet)
{
    fbe_payload_ex_t *                 sep_payload = NULL;
    ...
    fbe_transport_set_completion_function(new_packet, fbe_metadata_stripe_lock_control_entry_completion, debug_lock);                           

    /* Send a request to allocate the stripe lock. */
    status = fbe_stripe_lock_operation_entry(new_packet);
    return status;
}

static fbe_status_t
fbe_metadata_stripe_lock_control_entry_completion(fbe_packet_t * packet, fbe_packet_completion_context_t context)
{    
    fbe_payload_ex_t *                     sep_payload = NULL;
    fbe_payload_stripe_lock_operation_t *   stripe_lock_operation = NULL;
    fbe_payload_stripe_lock_status_t        stripe_lock_status;
...
fbe_status_t 
metadata_stripe_lock_drop_wait_queue_peer_requests(fbe_payload_stripe_lock_operation_t * sl)
{
    ...
    while (wait_queue_element != NULL) {
        ...
        if (wait_sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST) {
            ...
            fbe_spinlock_lock(&metadata_stripe_lock_peer_sl_queue_lock);
            fbe_queue_push(&metadata_stripe_lock_peer_sl_queue, &wait_sl->queue_element);
            metadata_stripe_lock_peer_sl_queue_count --;
            fbe_spinlock_unlock(&metadata_stripe_lock_peer_sl_queue_lock);
        }
    }
...
fbe_status_t 
metadata_stripe_lock_drop_peer_request(fbe_payload_stripe_lock_operation_t * sl, fbe_queue_head_t * tmp_queue)
{
    fbe_bool_t is_cmi_required = FBE_FALSE;
    fbe_metadata_element_t	* mde = NULL;	
...
fbe_status_t 
fbe_metadata_stripe_lock_peer_lost(void)
{
    fbe_queue_element_t		* queue_element = NULL;
    fbe_metadata_element_t	* mde = NULL;
    ...
    while (queue_element != NULL){
       ...
       /* Free the paged and Non-paged Slots */
        metadata_stripe_lock_get_paged_slots(mde, &first_slot, &last_slot);
        for(i = first_slot; i<= last_slot; i++) {
            metadata_stripe_lock_set_slot_state(mde, i, FBE_FALSE, METADATA_STRIPE_LOCK_SLOT_STATE_EXCLUSIVE_LOCAL);
        }
        ...
        */
        ...
        fbe_metadata_stripe_lock_peer_lost_mde(mde, &tmp_queue, &is_cmi_required);
...
static fbe_status_t 
fbe_metadata_stripe_grant_msg(fbe_metadata_element_t * mde, fbe_metadata_cmi_message_t * msg, fbe_u32_t cmi_message_length)
{
    ...
    if(!(cmi_stripe_lock->flags & FBE_METADATA_CMI_STRIPE_LOCK_FLAG_NEED_RELEASE)){
        /* Mark slots */
        metadata_stripe_lock_mark_grant_from_peer(mde, cmi_stripe_lock, sl);
    } else {
#if 0
        metadata_trace(FBE_TRACE_LEVEL_INFO,
                        FBE_TRACE_MESSAGE_ID_INFO,
...
static fbe_status_t 
fbe_metadata_stripe_lock_msg(fbe_metadata_element_t * mde, fbe_metadata_cmi_message_t * msg, fbe_u32_t cmi_message_length)
{
    ...
    peer_sl->cmi_stripe_lock.flags = 0;
    ...
    if(cmi_stripe_lock->header.metadata_cmi_message_type == FBE_METADATA_CMI_MESSAGE_TYPE_STRIPE_READ_LOCK){
        ...
        metadata_stripe_lock_set_invalid_region(&peer_sl->cmi_stripe_lock.write_region);
#if SL_DEBUG_MODE
        ...
        metadata_stripe_lock_set_invalid_region(&peer_sl->cmi_stripe_lock.read_region);
#if SL_DEBUG_MODE
        if(peer_sl->cmi_stripe_lock.header.object_id == 0x10c)
            metadata_trace(FBE_TRACE_LEVEL_INFO,
                            FBE_TRACE_MESSAGE_ID_FUNCTION_FAILED,
    ...
    fbe_spinlock_lock(&blob->peer_sl_queue_lock);
    //metadata_stripe_lock_update_write_count(peer_sl);
    fbe_queue_push(&blob->peer_sl_queue, &peer_sl->queue_element);
    fbe_spinlock_unlock(&blob->peer_sl_queue_lock);

    /* Submit it to the peer request Q */
    metadata_stripe_lock_reschedule_blob(mde, FBE_TRUE, FBE_FALSE);

    return FBE_STATUS_OK;
}

fbe_status_t 
metadata_stripe_lock_clear_wait_for_peer_requests(fbe_payload_stripe_lock_operation_t * sl)
{
    fbe_queue_element_t * wait_queue_element = NULL;
    fbe_payload_stripe_lock_operation_t * wait_sl = NULL;
...
/*!**************************************************************
 * fbe_stripe_lock_release_peer_data_stripe_locks()
 ****************************************************************
 * @brief
 ...
 ****************************************************************/
fbe_status_t 
fbe_stripe_lock_release_peer_data_stripe_locks(fbe_metadata_element_t* mde)
{
    fbe_queue_head_t tmp_queue;
    fbe_metadata_stripe_lock_blob_t * blob = NULL;  
    fbe_bool_t is_cmi_required = FBE_FALSE;
    ...
    fbe_stripe_lock_grant_all_peer_locks(mde, &tmp_queue, &is_cmi_required);
    ...
    if (is_cmi_required) {
        ...
        if (!fbe_queue_is_element_on_queue(&mde->stripe_lock_blob->queue_element)) {
            fbe_queue_push(&metadata_stripe_lock_blob_queue, &mde->stripe_lock_blob->queue_element);
            fbe_rendezvous_event_set(&metadata_stripe_lock_event);
        }
        fbe_spinlock_unlock(&metadata_stripe_lock_blob_queue_lock); 
    }
...
fbe_status_t 
fbe_metadata_stripe_lock_release_all_blobs(fbe_metadata_element_t * metadata_element)
{
...
fbe_bool_t
fbe_stripe_lock_was_paged_slot_held_by_peer(fbe_metadata_element_t  *mde)
{
    fbe_u32_t first_slot;
    fbe_u32_t last_slot;
    fbe_u32_t i;
    ...
    if (blob == NULL)
    {
        /* nothing we need to do if it hasn't been initialized */
        return FBE_FALSE;
    }
    ...
    for (i = first_slot; i<= last_slot; i++)
    {
        if(metadata_stripe_lock_get_slot_state(mde, i, FBE_FALSE) == METADATA_STRIPE_LOCK_SLOT_STATE_EXCLUSIVE_PEER)
        {
            return FBE_TRUE;
        }
    }
...
fbe_status_t 
fbe_metadata_stripe_lock_message_event(fbe_cmi_event_t event, fbe_metadata_cmi_message_t *metadata_cmi_msg, fbe_cmi_event_callback_context_t context)
{
    fbe_queue_head_t tmp_queue;
    fbe_metadata_element_t * mde = NULL;
    fbe_bool_t is_cmi_required = FBE_FALSE;
    ...
    if (fbe_base_config_metadata_element_to_class_id(mde) == FBE_CLASS_ID_EXTENT_POOL) {
        return fbe_ext_pool_lock_message_event(event, metadata_cmi_msg, context);
    }
...
#endif
        fbe_spinlock_lock(&metadata_stripe_lock_peer_sl_queue_lock);
        fbe_queue_push(&metadata_stripe_lock_peer_sl_queue, &peer_sl->queue_element);
        metadata_stripe_lock_peer_sl_queue_count--;
        fbe_spinlock_unlock(&metadata_stripe_lock_peer_sl_queue_lock);
        return FBE_STATUS_OK;
    }
...
static void metadata_stripe_lock_check_packet_queue(fbe_queue_head_t * queue)
{
    ...
    while(queue_element != NULL){
        ...
        metadata_stripe_lock_check_packet(packet);
        queue_element = fbe_queue_next(queue, queue_element);
    }/* while(queue_element != NULL) */
...
static void metadata_stripe_lock_check_packet(fbe_packet_t * packet)
{
    ...
    if(sl->opcode == FBE_PAYLOAD_STRIPE_LOCK_OPERATION_OPCODE_WRITE_UNLOCK ||
        ...
        sl->status != FBE_PAYLOAD_STRIPE_LOCK_STATUS_OK){
            if(!fbe_queue_is_empty(&sl->wait_queue)){
                metadata_trace(FBE_TRACE_LEVEL_CRITICAL_ERROR,
                                FBE_TRACE_MESSAGE_ID_FUNCTION_FAILED,
                                "SL: %p Wait queue is not empty\n",sl);
            }
    }
...
fbe_status_t 
fbe_metadata_element_stripe_lock_complete_cancelled(fbe_metadata_element_t * mde)
{
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    ...
    fbe_bool_t is_cmi_required = FBE_FALSE;
    fbe_queue_head_t * queue;
    fbe_u64_t queue_count;
    fbe_u32_t i;

    if (fbe_base_config_metadata_element_to_class_id(mde) == FBE_CLASS_ID_EXTENT_POOL) {
        /* Processing cancelled packet for extent pool is not enabled now */
        //fbe_metadata_element_ext_pool_lock_complete_cancelled(mde);
        return FBE_STATUS_OK;
    }
    ...
    if((mde->stripe_lock_hash != NULL) && !(mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_SL_HASH_DISABLED)){
        queue_count = mde->stripe_lock_hash->table_size;
    } else {
        queue_count = 1;
    }	
    ...
    for(i = 0 ; i < queue_count; i++){
        if(mde->stripe_lock_hash != NULL){
            queue = &mde->stripe_lock_hash->table[i].head;
            csx_p_spin_pointer_lock(&mde->stripe_lock_hash->table[i].lock);
            if(mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_SL_HASH_DISABLED){
                queue = &mde->stripe_lock_queue_head;
                fbe_spinlock_lock(&mde->stripe_lock_spinlock);
                queue_count = 1; /* We will do only one iteration */
            }			
        ...
        while(current_element != NULL){
            ...
            metadata_stripe_lock_complete_cancelled_queue(&sl->wait_queue, &tmp_queue);
        ...
        if(mde->stripe_lock_hash != NULL){
            ...
            csx_p_spin_pointer_unlock(&mde->stripe_lock_hash->table[i].lock);
        } else {			
            fbe_spinlock_unlock(&mde->stripe_lock_spinlock);
        }
...
static fbe_status_t 
metadata_stripe_lock_complete_cancelled_queue(fbe_queue_head_t * queue, fbe_queue_head_t * tmp_queue)
{
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_queue_element_t * current_element = NULL;
    fbe_metadata_element_t	* mde = NULL;
    ...
    while(current_element != NULL){
        ...
        if(!(sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST) && /* if it is not peer request */
                ...
                metadata_stripe_lock_dispatch(sl, &tmp_queue); 
                /* After dispatch we need to start from the front */
                current_element = fbe_queue_front(queue);
#endif
                fbe_payload_stripe_lock_set_status(sl, FBE_PAYLOAD_STRIPE_LOCK_STATUS_CANCELLED);
...
static fbe_status_t 
fbe_metadata_stripe_release_msg(fbe_metadata_element_t * mde, fbe_metadata_cmi_message_t * msg, fbe_u32_t cmi_message_length)
{
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_metadata_cmi_stripe_lock_t * cmi_stripe_lock = (fbe_metadata_cmi_stripe_lock_t *)msg;    
    fbe_queue_head_t tmp_queue;
...
static fbe_status_t 
fbe_metadata_stripe_aborted_msg(fbe_metadata_element_t * mde, fbe_metadata_cmi_message_t * msg, fbe_u32_t cmi_message_length)
{
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_metadata_cmi_stripe_lock_t * cmi_stripe_lock = (fbe_metadata_cmi_stripe_lock_t *)msg;    
    fbe_queue_head_t tmp_queue;
...
static fbe_status_t 
fbe_metadata_stripe_send_aborted_msg(fbe_metadata_element_t * mde, fbe_payload_stripe_lock_operation_t * sl)
{
    if(fbe_metadata_is_peer_object_alive(mde)){
...
static fbe_status_t 
fbe_metadata_stripe_send_release_msg(fbe_metadata_element_t * mde, fbe_payload_stripe_lock_operation_t * sl)
{
    if(fbe_metadata_is_peer_object_alive(mde)){
...
******************************************************************************/
fbe_status_t 
fbe_metadata_stripe_hash_init(fbe_u8_t * ptr, fbe_u32_t memory_size, fbe_u64_t default_divisor)
{
    fbe_u32_t i;
...
******************************************************************************/
fbe_status_t
fbe_metadata_stripe_hash_destroy(void * ptr)
{
    fbe_u32_t i;
...
fbe_status_t 
fbe_metadata_stripe_lock_all(fbe_metadata_element_t * mde)
{
    fbe_u32_t i;
...
fbe_status_t 
fbe_metadata_stripe_unlock_all(fbe_metadata_element_t * mde)
{
    fbe_u32_t i;
...
fbe_metadata_stripe_hash_get_entry(fbe_metadata_stripe_lock_hash_t * hash, fbe_payload_stripe_lock_operation_t * sl)
{
    ...
    /* Both paged and non paged SL will be on the last entry */
    if(!metadata_stripe_lock_is_np_lock(sl) && !metadata_stripe_lock_is_paged_lock(sl)){ /* User SL */
        key = (sl->stripe.first / hash->divisor) % (hash->table_size - 1);
    } else {
        key = hash->table_size - 1;
    }
...
{    
    ...
    while(queue_element != NULL){
        ...
        if(is_abort_allowed){ /* We are going to abort it */
            ...
            if(metadata_stripe_lock_is_paged_lock(sl)){
                fbe_metadata_stripe_lock_abort_paged_waiters(&sl->wait_queue, abort_queue);
            } else {
                while(abort_element = fbe_queue_pop(&sl->wait_queue)){ fbe_queue_push(abort_queue, abort_element); }
            }
...
fbe_status_t 
fbe_metadata_stripe_lock_hash(fbe_metadata_element_t * mde, fbe_payload_stripe_lock_operation_t * sl)
{
	if(mde->stripe_lock_hash != NULL){ /* lock_hash */
		fbe_metadata_stripe_lock_hash_table_entry_t * he;
		he = fbe_metadata_stripe_hash_get_entry(mde->stripe_lock_hash, sl);		
		csx_p_spin_pointer_lock(&he->lock);
		if(mde->attributes & FBE_METADATA_ELEMENT_ATTRIBUTES_SL_HASH_DISABLED){
			fbe_spinlock_lock( &mde->stripe_lock_spinlock);
		}
	} else {
...
fbe_status_t 
fbe_metadata_stripe_unlock_hash(fbe_metadata_element_t * mde, fbe_payload_stripe_lock_operation_t * sl)
{
    if(mde->stripe_lock_hash != NULL){ /* unlock_hash */
        fbe_metadata_stripe_lock_hash_table_entry_t * he;
        he = fbe_metadata_stripe_hash_get_entry(mde->stripe_lock_hash, sl);		
		...
		csx_p_spin_pointer_unlock(&he->lock);
	} else {
		fbe_spinlock_unlock( &mde->stripe_lock_spinlock);
	}
...
static fbe_status_t 
fbe_metadata_stripe_lock_peer_lost_mde(fbe_metadata_element_t * mde, fbe_queue_head_t * tmp_queue, fbe_bool_t * is_cmi_required)
{
    fbe_queue_element_t		* sl_queue_element = NULL;
    fbe_payload_stripe_lock_operation_t * sl = NULL;
    fbe_u32_t i;
    ...
    for(i = 0 ; i < queue_count; i++){
        ...
        while (sl_queue_element != NULL) {
            ...
            //if (sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_HOLDING_PEER) {
                ...
                metadata_stripe_lock_drop_wait_queue_peer_requests(sl);
            //}
            ...
            if (sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_REQUEST) {                
                if (!(sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_GRANTED)) {
                    metadata_stripe_lock_drop_peer_request(sl, tmp_queue);
                ...
                if((metadata_stripe_lock_is_np_lock(sl)) || (metadata_stripe_lock_is_paged_lock(sl))) { /* AR 568238 - Jornal flush needs paged lock. We are not jurnaling paged area */
            ...
            if((sl->status == FBE_PAYLOAD_STRIPE_LOCK_STATUS_PENDING) && ((metadata_stripe_lock_is_np_lock(sl)) || (metadata_stripe_lock_is_paged_lock(sl)))) {
                sl->flags &= ~FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_COLLISION;
             ...
             */
                if ((metadata_stripe_lock_is_np_lock(sl)) || (metadata_stripe_lock_is_paged_lock(sl))) {
                    sl->flags &= ~FBE_PAYLOAD_STRIPE_LOCK_FLAG_PEER_COLLISION;
                ...
                if (fbe_transport_is_monitor_packet(sl->cmi_stripe_lock.packet, fbe_base_config_metadata_element_to_object_id(mde)) ) {
                    ...
                    fbe_transport_record_callback_with_action(sl->cmi_stripe_lock.packet, (fbe_packet_completion_function_t) fbe_metadata_stripe_lock_peer_lost, PACKET_SL_ABORT);
                    fbe_payload_stripe_lock_set_status(sl, FBE_PAYLOAD_STRIPE_LOCK_STATUS_ABORTED);
                    //metadata_stripe_lock_update_write_count(sl);
                    fbe_transport_set_status(sl->cmi_stripe_lock.packet, FBE_STATUS_OK, 0);
                    if (fbe_queue_is_element_on_queue(&sl->cmi_stripe_lock.packet->queue_element)){
...
fbe_stripe_lock_grant_all_peer_locks(fbe_metadata_element_t* mde, fbe_queue_head_t * tmp_queue, fbe_bool_t * is_cmi_required)
{
    ...
    for(i = 0 ; i < queue_count; i++){
        ...
        while (sl_queue_element != NULL){
            ...
            metadata_stripe_lock_clear_wait_for_peer_requests(sl);
...
fbe_stripe_lock_scan_for_grants(fbe_metadata_element_t* mde, fbe_queue_head_t * grant_queue)
{
    ...
    if(mde->stripe_lock_hash != NULL){ /* Get queue count */
        queue_count = mde->stripe_lock_hash->table_size;
    } else {
        queue_count = 1;
    }	
    ...
    for(i = 0 ; i < queue_count; i++){
        ...
        while(queue_element != NULL){
            ...
            if(sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_GRANT && !(sl->flags & FBE_PAYLOAD_STRIPE_LOCK_FLAG_GRANTED)){			
                ...
                /* Allocate grant sl */
                fbe_spinlock_lock(&metadata_stripe_lock_peer_sl_queue_lock);
                grant_queue_element = fbe_queue_pop(&metadata_stripe_lock_peer_sl_queue);
                /* Fix for AR 550394 */
                if(grant_queue_element == NULL){ /* We out of memory */
                    metadata_trace(FBE_TRACE_LEVEL_WARNING,
                                   FBE_TRACE_MESSAGE_ID_INFO,
                                   "SL: metadata_stripe_lock_peer_sl_queue is EMPTY\n");
                    fbe_spinlock_unlock(&metadata_stripe_lock_peer_sl_queue_lock);
                    break;
                }
...
static fbe_status_t 
metadata_stripe_lock_disable_hash(fbe_metadata_element_t * mde)
{
    fbe_queue_element_t * queue_element;
    fbe_payload_stripe_lock_operation_t * sl;
    fbe_u32_t i;
    ...
    for(i = 0; i < mde->stripe_lock_hash->table_size; i++){
        while(queue_element = fbe_queue_pop(&mde->stripe_lock_hash->table[i].head)){
            sl = fbe_metadata_stripe_lock_queue_element_to_operation(queue_element);
            metadata_stripe_lock_push_sorted(&mde->stripe_lock_queue_head, sl);
        }
    }
...
static fbe_status_t 
metadata_stripe_lock_enable_hash(fbe_metadata_element_t * mde)
{
    fbe_queue_element_t * queue_element;
    fbe_payload_stripe_lock_operation_t * sl;
    fbe_metadata_stripe_lock_hash_table_entry_t * he;
    ...
    while(queue_element = fbe_queue_pop(&mde->stripe_lock_queue_head)){
        sl = fbe_metadata_stripe_lock_queue_element_to_operation(queue_element);
        he = fbe_metadata_stripe_hash_get_entry(mde->stripe_lock_hash, sl);
        metadata_stripe_lock_push_sorted(&he->head, sl);
    }
...
static fbe_status_t 
metadata_stripe_lock_push_sorted(fbe_queue_head_t * queue, fbe_payload_stripe_lock_operation_t * sl)
{
    fbe_queue_element_t * queue_element;
...
static fbe_status_t
metadata_stripe_lock_reschedule_blob(fbe_metadata_element_t * mde, fbe_bool_t is_cmi_required, fbe_bool_t is_hash_enable_required)
{
