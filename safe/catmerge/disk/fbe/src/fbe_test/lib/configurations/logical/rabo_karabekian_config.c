...
{
#define NUMBER_OF_OBJECTS 14 /* 1 board, 1 port, 1 enclosure, 11 PDOs */
    ...
    if(fbe_test_util_is_simulation())
    {
        ...
        fbe_u32_t i;
        fbe_u32_t num_drives = 11;
        fbe_u32_t total_objects = 0;
        fbe_class_id_t class_id;

        fbe_status_t status;

        mut_printf(MUT_LOG_LOW, "=== init Rabo_Karabekian configuration ===\n");
...
{
    mut_printf(MUT_LOG_LOW, "=== Rabo Karabekian destroy configuration ===\n");
    if(fbe_test_util_is_simulation())
    {
