 ...
 *
 * @brief
 *  This file contains the test for power saving enable/disbale
 *
 *
 ***************************************************************************/
 ...
 *   TEST DESCRIPTION
 *************************/
char * boomer_short_desc = "Power saving capabilities(single SP)";
char * boomer_long_desc =
"The Power Saving Scenario (boomer) tests the ability of the system to enable or disable power saving and other PS related functions\n"
"It covers the following cases: \n"
"- The system wide power syaving is enabled, and disbaled and verification make sure all objects are hibernating and then getting out\n"
"- Object based power syaving is enabled, and disbaled and verification make sure all objects are\n"
...
static fbe_api_rdgen_context_t 	boomer_test_rdgen_contexts[5];
static fbe_semaphore_t          sem;
static fbe_notification_registration_id_t          reg_id;

static void boomer_test_create_lun(fbe_test_rg_configuration_t *rg_config_p, fbe_lun_number_t lun_number);
static void boomer_test_destroy_lun(fbe_lun_number_t lun_number);
static void boomer_commmand_response_callback_function (update_object_msg_t * update_object_msg, void *context);
static void boomer_test_write_background_pattern(fbe_element_size_t element_size, fbe_object_id_t lun_object_id);

static void boomer_test_setup_rdgen_test_context(  fbe_api_rdgen_context_t *context_p,
...
static fbe_status_t boomer_test_run_io_load(fbe_object_id_t lun_object_id);

static void boomer_test_setup_fill_rdgen_test_context(fbe_api_rdgen_context_t *context_p,
                                                    fbe_object_id_t object_id,
                                                    fbe_class_id_t class_id,
                                                    fbe_rdgen_operation_t rdgen_operation,
                                                    fbe_lba_t max_lba,
...
static void boomer_test_verify_system_enable_disable(void);
static void boomer_test_verify_object_enable_disable(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_background_io_not_count(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_power_saving_policy(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_bound_objects_save_power(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_unbound_objects_save_power(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_objects_get_out_of_power_save(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_periodic_wakeup(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_io_wakeup(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_power_save_stat(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_inject_not_spinning_error(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_load_physical_config(void);
static void boomer_test_create_rg(fbe_test_rg_configuration_t *rg_config_p);
static void boomer_test_verify_lun_creation_on_a_hibernating_rg(fbe_test_rg_configuration_t *rg_config_p);

/**********************************************************************************/
#define BOOMER_TEST_LUNS_PER_RAID_GROUP         2
#define BOOMER_TEST_CHUNKS_PER_LUN              1
#define BOOMER_TEST_ENCL_MAX                    2
#define BOOMER_TEST_NUMBER_OF_PHYSICAL_OBJECTS  34 /* (1 board + 1 port + 2 encl + 30 pdo) */
#define BOOMER_JOB_SERVICE_CREATION_TIMEOUT     150000 /* in ms*/
//#define BOOMER_TEST_LUNS_PER_RAID_GROUP         1

static fbe_test_rg_configuration_t boomer_rg_config[] = 
{
...
static void boomer_reboot_sp(void)
{
    fbe_status_t status = FBE_STATUS_GENERIC_FAILURE;
    /*reboot sep*/
    fbe_test_sep_util_destroy_neit_sep();
    sep_config_load_sep_and_neit();
    status = fbe_test_sep_util_wait_for_database_service(BOOMER_JOB_SERVICE_CREATION_TIMEOUT);
    MUT_ASSERT_INT_EQUAL(status, FBE_STATUS_OK);
}

/*!**************************************************************
 * boomer_test_load_physical_config()
 ****************************************************************
 * @brief
 *  Configure the Boomer test physical configuration.
 *
 * @param None.              
 *
 * @return None.
...
static void boomer_test_load_physical_config(void)
{
    ...
    fbe_api_terminator_device_handle_t      port0_encl_handle[BOOMER_TEST_ENCL_MAX];
     ...
     */
    for ( enclosure = 0; enclosure < BOOMER_TEST_ENCL_MAX; enclosure++)
    { 
    ...
    status = fbe_api_wait_for_number_of_objects(BOOMER_TEST_NUMBER_OF_PHYSICAL_OBJECTS, 20000, FBE_PACKAGE_ID_PHYSICAL);
    ...
    MUT_ASSERT_TRUE(total_objects == BOOMER_TEST_NUMBER_OF_PHYSICAL_OBJECTS);
    mut_printf(MUT_LOG_TEST_STATUS, "verifying object count...Passed\n");

    return;

} /* End boomer_test_load_physical_config() */
...
static fbe_status_t boomer_test_run_io_load(fbe_object_id_t lun_object_id)
{
    fbe_api_rdgen_context_t*    context_p = &boomer_test_rdgen_contexts[0];
    fbe_status_t                status;


    /* Write a background pattern; necessary until LUN zeroing is fully supported */
    boomer_test_write_background_pattern(128, lun_object_id);

    /* Set up for issuing reads forever
     */
    boomer_test_setup_rdgen_test_context(context_p,
                                         lun_object_id,
...
static void boomer_test_write_background_pattern(fbe_element_size_t element_size, fbe_object_id_t lun_object_id)
{
     ...
     */
    boomer_test_setup_fill_rdgen_test_context( context_p,
                                                    lun_object_id,
                                                    FBE_CLASS_ID_INVALID,
...
} /* end boomer_test_write_background_pattern() */
...
static void boomer_test_verify_power_saving_policy(fbe_test_rg_configuration_t *rg_config_p)
{
    ...
    for (lu_index = 0; lu_index < BOOMER_TEST_LUNS_PER_RAID_GROUP; lu_index++){
...
static void boomer_test_verify_io_wakeup(fbe_test_rg_configuration_t *rg_config_p)
{
    ...
    boomer_test_run_io_load(lu_object_id);
...
static void boomer_test_create_rg(fbe_test_rg_configuration_t *rg_config_p)
{
    ...
    status = fbe_api_common_wait_for_job(rg_config_p->job_number,
                                         BOOMER_JOB_SERVICE_CREATION_TIMEOUT,
                                         &job_error_code,
                                         &job_status,
...
} /* End boomer_test_create_rg() */

static void boomer_test_verify_system_power_save_after_reboot(void)
{
    fbe_system_power_saving_info_t              power_save_info;
    fbe_status_t                                status;
 ...
 *
 * @brief
 *  This is the main executing function of the Boomer test.
 *
 * @param
 *
 ...
 * 
 * @Modified
 * 29/08/2011 - Created. Sandeep Chaudhari
 *****************************************************************************/
void boomer_run_test(fbe_test_rg_configuration_t *rg_config_ptr ,void * context_p)
{
    fbe_status_t                status = FBE_STATUS_GENERIC_FAILURE;
    ...
    for(rg_index = 0; rg_index < num_raid_groups; rg_index++)
    {
        ...
        status = fbe_api_notification_interface_register_notification(FBE_NOTIFICATION_TYPE_LIFECYCLE_STATE_HIBERNATE | FBE_NOTIFICATION_TYPE_LIFECYCLE_STATE_READY,
                                                                      ...
                                                                      boomer_commmand_response_callback_function,
                                                                      &sem,
                                                                      &reg_id);

        MUT_ASSERT_INT_EQUAL(FBE_STATUS_OK, status);

        boomer_test_inject_not_spinning_error(rg_config_p);
        boomer_test_verify_system_enable_disable();
        boomer_test_verify_object_enable_disable(rg_config_p);
        boomer_test_verify_background_io_not_count(rg_config_p);
        boomer_test_verify_power_saving_policy(rg_config_p);
        boomer_test_verify_bound_objects_save_power(rg_config_p);
        boomer_test_verify_unbound_objects_save_power(rg_config_p);
        boomer_test_verify_objects_get_out_of_power_save(rg_config_p);
        boomer_test_verify_periodic_wakeup(rg_config_p);
        boomer_test_verify_io_wakeup(rg_config_p);
        boomer_test_verify_lun_creation_on_a_hibernating_rg(rg_config_p);
        ...
        boomer_test_verify_power_save_stat(rg_config_p); 
        ...
        boomer_test_verify_objects_get_out_of_power_save(rg_config_p);
        boomer_test_verify_system_power_save_after_reboot();

        status = fbe_api_notification_interface_unregister_notification(boomer_commmand_response_callback_function, reg_id);
        MUT_ASSERT_TRUE(status == FBE_STATUS_OK);
 ...
 ******************************************************************************
 * @brief
 *  This is the main entry point for the Boomer test.  
 *
 * @param None.
 *
 * @return None.
...
{
    fbe_test_rg_configuration_t *rg_config_p = NULL;
    rg_config_p = &boomer_rg_config[0];

    mut_printf(MUT_LOG_TEST_STATUS, "=== Called boomer_test ===\n");
	
    fbe_test_sep_util_init_rg_configuration_array(rg_config_p);
    fbe_test_run_test_on_rg_config(rg_config_p, NULL, boomer_run_test, 
                                    BOOMER_TEST_LUNS_PER_RAID_GROUP, 
                                    BOOMER_TEST_CHUNKS_PER_LUN);
 ...
 ******************************************************************************
 * @brief
 *  This is the setup function for the boomer test. It is responsible
 ...
 *
 * @author
 *  8/29/2011 - Created. Sandeep Chaudhari
 ******************************************************************************/
void boomer_test_setup(void)
{
    mut_printf(MUT_LOG_TEST_STATUS, "=== Setup for Boomer test ===\n");
    ...
    return;
}/* End boomer_test_setup() */


/******************************************************************************
 *
 * @brief
 *  This is the cleanup function for the Boomer test.
 *
 * @param   None
 *
 * @return  None
...
{  
    ...
    mut_printf(MUT_LOG_TEST_STATUS, "=== Cleanup for Boomer Test ===\n");
