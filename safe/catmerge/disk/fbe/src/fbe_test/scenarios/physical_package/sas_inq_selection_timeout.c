...
void sas_inq_selection_timeout_test(void)
{
    ...
    fbe_api_terminator_device_handle_t  encl_id1, drive_id1, drive_id2;
    ...
    sas_encl.uid[0] = sas_encl.encl_number;
    ...
    MUT_ASSERT_TRUE(status == FBE_STATUS_OK);
	/* Set drive state to return SELECTION_TIMEOUT on INQ command */
	fbe_api_terminator_drive_set_state (drive_id1,TERMINATOR_SAS_DRIVE_SELECT_TIMEOUT);
    ...
    MUT_ASSERT_TRUE(status == FBE_STATUS_OK);
	/* Set drive state to return SELECTION_TIMEOUT on INQ command */
	fbe_api_terminator_drive_set_state (drive_id2,TERMINATOR_SAS_DRIVE_SELECT_TIMEOUT);
    ...
    status = fbe_api_wait_for_number_of_objects(STATIC_NUMBER_OF_OBJ, 20000, FBE_PACKAGE_ID_PHYSICAL); /* no logical drives */
    ...
    for (port_idx = 0; port_idx < FBE_TEST_PHYSICAL_BUS_COUNT; port_idx++) {
        for (enclosure_idx = 0; enclosure_idx < FBE_TEST_ENCLOSURES_PER_BUS; enclosure_idx++) {
            for (drive_idx = 0; drive_idx < FBE_TEST_ENCLOSURE_SLOTS; drive_idx++) {
                ...
                {
                    /* Specialize timer should fail object within 120 sec. */
					EmcutilSleep(130000);/* 130 sec to be on a safe side */
                    ...
                    status = fbe_test_pp_util_verify_pdo_state(port_idx, enclosure_idx, drive_idx, 
                                                                FBE_LIFECYCLE_STATE_FAIL, 10000);
                    MUT_ASSERT_TRUE(status == FBE_STATUS_OK);
                } else {
    ...
    /*let's see how many objects we have, at this stage we should have 5 objects:
    1 board
    1 port
    1 enclosure
    2 physical drives
