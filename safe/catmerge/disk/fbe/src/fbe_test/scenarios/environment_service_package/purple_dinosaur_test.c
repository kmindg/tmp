 ...
 *   MACRO DEFINITIONS
 *************************/
#define PURPLE_DINOSAUR_BUS  0
#define PURPLE_DINOSAUR_ENCL 0
#define PURPLE_DINOSAUR_SLOT 5
#define PURPLE_DINOSAUR_LIFECYCLE_NOTIFICATION_TIMEOUT 15000  // 15 sec
 ...
 *************************/
/*
static void purple_dinosaur_intermediate_destroy(void);
static void purple_dinosaur_intermediate_start(void);
*/
static void purple_dinosaur_intermediate_restart(void);
static void test_valid_invalid_drive_class_current_support(void);
/*******************************************************************************************/ 
    ...
    */
    ...
    if ( state == FBE_LIFECYCLE_STATE_DESTROY)   // special case since object id is destroyed.  can't do normal wait.
    {
        ...
        for (i=0; i<15000; i+= 100)    // 15 sec timeout
        {
             ...
             */            
            status = fbe_api_get_physical_drive_object_id_by_location(PURPLE_DINOSAUR_BUS, PURPLE_DINOSAUR_ENCL, PURPLE_DINOSAUR_SLOT, &object_handle_p);
    ...
    {    
        status = fbe_test_pp_util_verify_pdo_state(PURPLE_DINOSAUR_BUS, 
                                                   PURPLE_DINOSAUR_ENCL, 
                                                   PURPLE_DINOSAUR_SLOT, 
                                                   state, 
                                                   PURPLE_DINOSAUR_LIFECYCLE_NOTIFICATION_TIMEOUT);
        ...
        if ( state != FBE_LIFECYCLE_STATE_FAIL )
        {
            ...
            status = fbe_test_pp_util_verify_ldo_state(PURPLE_DINOSAUR_BUS, 
                                                       PURPLE_DINOSAUR_ENCL, 
                                                       PURPLE_DINOSAUR_SLOT, 
                                                       state, 
                                                       PURPLE_DINOSAUR_LIFECYCLE_NOTIFICATION_TIMEOUT); 
...
/*******************************************************************************************/
static fbe_status_t purple_dinosaur_insert_drive(const TEXT* product_id, fbe_lba_t capacity)
{
    ...
    drive_info.location.bus =       PURPLE_DINOSAUR_BUS;
    drive_info.location.enclosure = PURPLE_DINOSAUR_ENCL;
    drive_info.location.slot =      PURPLE_DINOSAUR_SLOT;    
    
    status = fbe_api_terminator_get_enclosure_handle(PURPLE_DINOSAUR_BUS,PURPLE_DINOSAUR_ENCL,&encl_handle);
    MUT_ASSERT_TRUE(status == FBE_STATUS_OK);
    
    sas_drive.backend_number = PURPLE_DINOSAUR_BUS;
    sas_drive.encl_number = PURPLE_DINOSAUR_ENCL;
    sas_drive.capacity = capacity;
    sas_drive.block_size = 520;
    sas_drive.sas_address = 0x5000097A78000000 + ((fbe_u64_t)(sas_drive.encl_number + sas_drive.backend_number) << 16) + PURPLE_DINOSAUR_SLOT;
    ...
    mut_printf(MUT_LOG_LOW, "Inserting drive %d_%d_%d\n", sas_drive.backend_number, sas_drive.encl_number,
               PURPLE_DINOSAUR_SLOT);    
    status = fbe_api_terminator_insert_sas_drive (encl_handle, PURPLE_DINOSAUR_SLOT, &sas_drive, &drive_handle); 
    MUT_ASSERT_TRUE(status == FBE_STATUS_OK);
/*
...
/*******************************************************************************************/
static fbe_status_t purple_dinosaur_remove_drive(fbe_u32_t port,
                                                 fbe_u32_t enclosure,
                                                 fbe_u32_t slot)
{
    fbe_esp_drive_mgmt_drive_info_t     drive_info;
...
static void test_valid_cap_known_b_class_drive(TEXT* product_id, fbe_lba_t capacity)
{
    ...
    status = purple_dinosaur_remove_drive(PURPLE_DINOSAUR_BUS,PURPLE_DINOSAUR_ENCL,PURPLE_DINOSAUR_SLOT);
    MUT_ASSERT_TRUE(status == FBE_STATUS_OK);   
    mut_printf(MUT_LOG_LOW, "Insertion B class drive (id=%s cap=%X)\n", product_id+8,(unsigned int)capacity);
    status = purple_dinosaur_insert_drive(product_id, capacity);   // expect pass
    MUT_ASSERT_TRUE(status == FBE_STATUS_OK); 
    wait_for_state(FBE_LIFECYCLE_STATE_READY);

    /* Test that event log contains msg for drive coming online
     */
    status = fbe_api_get_physical_drive_object_id_by_location(PURPLE_DINOSAUR_BUS, PURPLE_DINOSAUR_ENCL, PURPLE_DINOSAUR_SLOT, &object_id);
    MUT_ASSERT_INT_EQUAL(status, FBE_STATUS_OK);
...
static void test_valid_cap_known_j_class_drive(TEXT* product_id, fbe_lba_t capacity)
{
    ...
    status = purple_dinosaur_insert_drive(product_id, capacity);  // expect fail
    MUT_ASSERT_TRUE(status == FBE_STATUS_OK); 
...
static void test_valid_invalid_drive_class_current_support(void)
{
    ...
    */
    ...
    mut_printf(MUT_LOG_LOW, "Checking for J class support on Defiant M4.");
    ...
    mut_printf(MUT_LOG_LOW, "Success.. Checking for J class support on Defiant M4.");

    mut_printf(MUT_LOG_LOW, "Checking for J class support on Defiant M5.");
    drive_supported = FBE_FALSE;
    ...
    mut_printf(MUT_LOG_LOW, "Success.. Checking for J class support on Defiant M5.");

    mut_printf(MUT_LOG_LOW, "Checking for J class support on Prometheus.");
    /* FBE_DRIVE_PRICE_CLASS_J should not be supported on SPID_MUSTANG_HW_TYPE. */
    ...
    mut_printf(MUT_LOG_LOW, "Success..Checking for J class support on Prometheus.");

    mut_printf(MUT_LOG_LOW, "Checking for B class support on different platforms.");
    drive_supported = FBE_FALSE;
    ...
    mut_printf(MUT_LOG_LOW, "Success..Checking for B class support on different platforms.");

}


/*******************************************************************************************/
char * purple_dinosaur_short_desc = "DMO persistence test";
char * purple_dinosaur_long_desc ="\
Tests the DMO persistence feature for drive price class table.\n\
\n\
\n\
Starting Config:\n\
        [PP] armada board\n\
...
STEP 2: Test table entry addition, lookup and deletion. Reboot after addition or deletion.\n\
 ...
 ****************************************************************/
void purple_dinosaur_test(void)
{
    fbe_drive_mgmt_policy_t policy;
    fbe_status_t status = FBE_STATUS_OK;
    fbe_bool_t   drive_supported = FBE_FALSE;
   ...
   /* Wait util there is no firmware upgrade in progress. */
    ...
    mut_printf(MUT_LOG_LOW, "Adding J class drive entry for Defiant M4.");
    ...
    mut_printf(MUT_LOG_LOW, "Adding J class drive entry for Defiant M5.");
    status = fbe_api_esp_drive_mgmt_add_drive_price_class_support_entry(FBE_DRIVE_PRICE_CLASS_J, SPID_DEFIANT_M5_HW_TYPE);
    MUT_ASSERT_TRUE_MSG(status == FBE_STATUS_OK, "Adding price class - platform entry failed!");

    /* Test whether the entries were added and used correctly.*/
    test_valid_invalid_drive_class_current_support();

    /* Sleep to allow persistence. */
    fbe_api_sleep(6000);

    /* Restart stack to confirm the information has been persisted.*/
    mut_printf(MUT_LOG_LOW, "=== Restarting stack to check for persistence.  ===");
/*
    mut_printf(MUT_LOG_LOW, "=== Starting stack shutdown.");
    purple_dinosaur_intermediate_destroy();


    mut_printf(MUT_LOG_LOW, "=== Starting stack restart.");
    purple_dinosaur_intermediate_start();
*/ 
    purple_dinosaur_intermediate_restart();
    mut_printf(MUT_LOG_LOW, "=== Stack restart completed.");
    /* Test whether the entries were added, persisted, read back and used correctly.*/
    test_valid_invalid_drive_class_current_support();
    ...
    MUT_ASSERT_TRUE_MSG(status == FBE_STATUS_OK, "Removing price class - platform entry failed!");

    /* Should return not supported since only a single entry should exist for 
     * FBE_DRIVE_PRICE_CLASS_J - SPID_HELLCAT_LITE_HW_TYPE and we just removed it.
     */
    drive_supported = FBE_TRUE;
    status = fbe_api_esp_drive_mgmt_check_drive_price_class_supported(FBE_DRIVE_PRICE_CLASS_J, SPID_DEFIANT_M5_HW_TYPE, &drive_supported);
    MUT_ASSERT_TRUE_MSG(status == FBE_STATUS_OK, "Checking price class - platform supported failed!");
...
/******************************************
 * end purple_dinosaur_test()
 ******************************************/

void purple_dinosaur_setup(void)
{
...
void purple_dinosaur_destroy(void)
{
    fbe_test_sep_util_destroy_neit_sep_physical();
    fbe_test_esp_delete_registry_file();
    fbe_test_esp_delete_dmo_lun();
    return;
}
/*
static void purple_dinosaur_intermediate_destroy(void)
...
/*************************
 * end file purple_dinosaur_test.c
 *************************/
