 ...
 *
 * @brief
 *  This file contains the functions to test Single Jetfire SP BM firmware upgrade
...
char * garnett_short_desc = "Test Single Jetfire SP BM firmware upgrade state transition and queuing logic.";
char * garnett_long_desc =
    "\n"
    "\n"
    "The garnett scenario tests Single Jetfire SP BM firmware upgrade state transition and queuing logic.\n"
    "It includes: \n"
...
//    fbe_api_sim_transport_set_target_server(FBE_SIM_SP_A);
    ...
    fbe_test_esp_create_image_file(FBE_DEVICE_TYPE_BACK_END_MODULE);
    fbe_test_esp_create_registry_image_path(FBE_DEVICE_TYPE_BACK_END_MODULE);
...
//    fbe_api_sim_transport_set_target_server(FBE_SIM_SP_A);
    ...
    fbe_test_esp_delete_image_file(FBE_DEVICE_TYPE_BACK_END_MODULE);
    fbe_test_esp_delete_image_file(FBE_DEVICE_TYPE_LCC);

    return;
}

fbe_status_t fbe_test_load_garnett_config(SPID_HW_TYPE platform_type)
{
...
//    fbe_api_sim_transport_set_target_server(FBE_SIM_SP_A);
    ...
    status = fbe_test_load_garnett_config(SPID_DEFIANT_M1_HW_TYPE);
    MUT_ASSERT_TRUE(status == FBE_STATUS_OK);
    // debug only
    mut_printf(MUT_LOG_LOW, "=== SPA config loading is done ===\n");

//    garnett_load_specl_config();
