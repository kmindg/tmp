...
#if !defined(OPTION_H_NO_CSTDLIB)
# include <cstdlib>
#endif

#if defined(SHMEM_DLL_EXPORT)
#define OPTION_CLASSPUBLIC CSX_CLASS_EXPORT
#else
#define OPTION_CLASSPUBLIC CSX_CLASS_IMPORT
#endif
...
class OPTION_CLASSPUBLIC Options {
public:
    virtual ~Options();
...
class OPTION_CLASSPUBLIC Program_Option {
...
private:
    // routine to recored the options being defined and to ensure duplicates are removed.
    void recordOption();
};

// use this for an integer type option with a default value of 0
class OPTION_CLASSPUBLIC Int_option : public Program_Option {
private:
    char valueStr[20];
    int value;
...
// use this for an integer type option with a default value of 0
class OPTION_CLASSPUBLIC HexInt_option : public Program_Option {
private:
    char valueStr[20];
    int value;
    char **mOptionValue;
...
public:
    ...
    HexInt_option(const char* name, const char* arg, const char *help_txt = "", int default_value = 0, bool _show = true, bool record = true);
    HexInt_option(const char* name, const char* arg, int num_of_args, bool show, const char *help_txt, bool record = true, int default_value = 0);
    ~HexInt_option();
...
// use this for an integer type option with a default value of 0
class OPTION_CLASSPUBLIC HexInt64_option : public Program_Option {
private:
    char valueStr[CSX_NUMBER_MAX_CHARS];
    csx_u64_t value;
    char **mOptionValue;


public:
    virtual void _set(csx_u64_t v);// direct setter for value for derived classes
    virtual void set(char *arg);

    HexInt64_option(const char* name, const char* arg, const char *help_txt = "", csx_u64_t default_value = 0, bool _show = true, bool record = true);
    HexInt64_option(const char* name, const char* arg, int num_of_args, bool show, const char *help_txt, bool record = true, csx_u64_t default_value = 0);
    ~HexInt64_option();
...
// string value option
class OPTION_CLASSPUBLIC String_option : public Program_Option{
private:
    char valueStr[1024];
    // char *value;
protected:
...
class OPTION_CLASSPUBLIC Bool_option : public Program_Option {
private:
    bool value;
