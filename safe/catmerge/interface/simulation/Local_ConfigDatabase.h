...
public:
    ...
    //                   the value remains open. Return value undefined if the ValueName
    //                   does not exist.
    // @param ValueLen - the number of bytes stored in ValuePtr.  Returned as 0 if the
    //                   ValueName does not exist.
    virtual VOID GetBinaryValue(const PCHAR ValueName, PVOID & ValuePtr,  ULONG & ValueLen) const = 0;
    ...
    static void RegisterDatabase(DatabaseKey *database);
#endif
